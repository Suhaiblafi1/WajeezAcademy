/* أصل الموقع القانوني — مصدر واحد للنطاق الذي نقول للعالم إنه بيتنا.
   ...
   النطاق الحيّ www.wajeezacademy.com، والموقع عليه اليوم على خادم Hetzner
   واحد: حاوياتُ Docker خلف Caddy، ويُنشر بـ`deploy/deploy.sh` سحبا من GitHub. وكان الأصل مكتوبا حرفا في مكانين — SeoHead
   ...
   والترتيب هنا يصحّح نفسه بلا تدخل:
     ١) VITE_SITE_ORIGIN إن ضُبط وقت البناء — الكلمة الأخيرة للمالك، وهو
        الموضع الصحيح لضبطه: `SITE_DOMAIN` في `deploy/.env.production`،
        ومنه يشتقّه البناءُ داخل الصورة (`deploy/compose.prod.yml`).
