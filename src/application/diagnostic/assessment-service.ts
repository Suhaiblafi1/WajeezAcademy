...
/** ما يعرفه المحرك عن هذه اللحظة بلغة المتعلم — أسماءٌ لا رموز */
...
/* الجملة العامة كانت واحدة لكل الحالات: «مساران يناسبانك حتى الآن — إجابتك
   ترجّح أحدهما». وهي لا تسمّي المسارين ولا المهارة، فلا تشرح شيئا: يقرؤها
   المتعلم مرة ثم يتجاهل الصندوق تحت كل سؤال.
   والمحرك يعرف الأسماء الثلاثة، فصارت كل ترجمة دالّةً فيها: تُركَّب الجملة
   المحدّدة متى توفّر السياق، وتسقط إلى العامة متى غاب — لا فراغَ ولا «undefined»
   في جملة تُعرض. */
type WhyFn = (c: WhyContext) => string
const q = (x?: string) => (x ? `«${x}»` : null)

const WHY_AR: Record<string, WhyFn> = {
  'يكمل سياقًا حاسمًا للقرار': (c) =>
    q(c.topTitle_ar) ? `يكمل سياقًا يتوقّف عليه ترشيح ${q(c.topTitle_ar)} لك.` : 'يكمل سياقًا يؤثّر في ترشيحك مباشرة.',
  'يفصل غموض المجال': () => 'مجالك ما زال غير محسوم — إجابتك توضّحه.',
  'يفصل بين المرشحين المتصدرين بدليل مهارة': (c) =>
    c.skillName_ar && c.topTitle_ar && c.rivalTitle_ar
      ? `${q(c.skillName_ar)} تفصل بين ${q(c.topTitle_ar)} و${q(c.rivalTitle_ar)} — إجابتك ترجّح أحدهما.`
      : 'مساران يناسبانك حتى الآن — إجابتك ترجّح أحدهما.',
  'يقيس مهارة حاسمة للمتصدر قبل أي اكتفاء': (c) =>
    c.skillName_ar && c.topTitle_ar
      ? `${q(c.skillName_ar)} مهارة أساسية في ${q(c.topTitle_ar)} — إجابتك تحدّد أتبدأ من أساسياتها أم من تطبيقاتها.`
      : 'نقيس مهارة أساسية في المسار الأقرب لك قبل أن نرشّحه.',
  ...
  'يقيس مهارة يتطلبها المرشح المتصدر': (c) =>
    c.skillName_ar && c.topTitle_ar
      ? `${q(c.topTitle_ar)} يتطلب ${q(c.skillName_ar)} — نقيس مستواك فيها لنرتّب دوراتك عليه.`
      : 'نقيس مهارة يتطلبها المسار الأقرب لك.',
}

/** العبارات الخاصة التي يكتبها المحرك كاملة بدل التذييل */
const SPECIAL_AR: [RegExp, WhyFn][] = [
  [/مهارة حاسمة لأحد مرشحي الصدارة ما زالت مجهولة/, (c) =>
    c.skillName_ar
      ? `${q(c.skillName_ar)} مهارة أساسية في أحد المسارات المرشّحة لك ولم تُقَس بعد — نقيسها قبل الترشيح.`
      : 'مهارة أساسية في أحد المسارين المرشّحين لك ما زالت غير مقيسة — نقيسها قبل الترشيح.'],
  [/خطة مركبة منافسة .* تنقصها حقيقة مطلوبة/, (c) =>
    q(c.rivalTitle_ar)
      ? `خطة ${q(c.rivalTitle_ar)} قد تناسبك أكثر — تنقصها معلومة واحدة قبل أن نُنصفها.`
      : 'خطة مركّبة قد تناسبك أكثر — تنقصها معلومة واحدة قبل أن نُنصفها.'],
  [/مهارة حاسمة للمرشح المتصدر ما زالت مجهولة/, (c) =>
    c.skillName_ar && c.topTitle_ar
      ? `لا نرشّح ${q(c.topTitle_ar)} بلا دليل واحد على الأقل — و${q(c.skillName_ar)} هي هذا الدليل.`
      : 'لا نرشّح مسارًا بلا دليل مهاري واحد على الأقل — وهذا هو.'],
]

export function learnerWhy(raw: string, ctx: WhyContext = {}): string | null {
  const s = raw.trim()
  if (s.length === 0 || GENERIC_WHY.test(s)) return null

  const framed = s.match(/^فاز أساسًا بسبب:\s*(.+?)\.?$/)
  if (framed) return WHY_AR[framed[1].trim()]?.(ctx) ?? null

  for (const [re, fn] of SPECIAL_AR) if (re.test(s)) return fn(ctx)
...
function whyOfLastQuestion(trace: { kind: string; data?: Record<string, unknown> }[]): string | null {
  ...
  const raw = entry?.data?.whyContext
  const ctx: WhyContext = raw && typeof raw === 'object' ? (raw as WhyContext) : {}
  return learnerWhy(reason, ctx)
}
