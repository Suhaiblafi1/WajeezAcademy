...
/* ═══════════ وقفُ الحجز المؤقّت — قرارُ صاحب المنصّة (٢٠ سبتمبر ٢٠٢٦) ═══════════
   ...
   يقرؤه أربعةٌ معا: بطاقةُ الحجز، وشاشةُ ما بعد الإرسال، وشرحُ حالة «مقدَّم»
   أدناه، ودعوةُ الحجز في `interview-invitation.ts` (بطاقةً وبريدا). ولو كُتب
   في البطاقة وحدَها لبقيت الشاشاتُ تَعِد بتقويمٍ لا يُعرض.

   ═══ وأُطفئ — والمفتاحُ يبقى (٢٢ سبتمبر ٢٠٢٦) ═══

   أمر صاحبُ المنصّة: «اطفئ الوقف» — فُتحت المواعيد وعاد التقويمُ يُعرض،
   وعادت معه الشاشاتُ الأربعُ إلى ألفاظها الأولى بهذا السطر وحدَه، كما كُتب
   فوقُ أنّها تعود.

   **ولا يُحذف المفتاح**: الامتلاءُ حالٌ تتكرّر في منصّةٍ يكثر متقدّموها،
   ومن حذفه اليومَ كتبه ثانيةً في أوّل شهرٍ يمتلئ — وكتبه ناقصا (شاشةٌ تُنسى،
   أو عودةٌ بلا شهرٍ مسمّى). و`monthAr` يبقى على «سبتمبر» سجلًّا للشهر الذي
   امتلأ؛ ومن أوقف الحجزَ مرّةً أخرى بدّلهما معه — لا يُعاد بناءُ الوقف. */
export const INTERVIEW_BOOKING_PAUSE = {
  /** أُطفئ ٢٢ سبتمبر ٢٠٢٦ — ويُوقَد وحدَه يومَ تمتلئ المواعيد مرّةً أخرى */
  active: false,
...
/* ═══════════ الحجزُ بدعوةٍ وحدَها — قرارُ صاحب المنصّة (٢٤ سبتمبر ٢٠٢٦) ═══════════

   «أوقِف الوصولَ للحجز من خلال تقديم الطلب، واتركه مفتوحا لمن نرسل له إيميلَ
   تحديد موعدٍ أو رسالةَ اطمئنانٍ فيها رابطُ الدعوة.»

   ═══ وهو غيرُ `INTERVIEW_BOOKING_PAUSE` — بابان لا بابٌ واحد ═══

   الوقفُ يُغلق التقويمَ **على الناس جميعا** شهرا ثمّ يُفتح: حالٌ طارئةٌ
   تمرّ. وهذا يُغلقه على **بابٍ واحدٍ بعينه** — بابِ من قدّم طلبا للتوّ —
   ويتركه مفتوحا لمن دعوناه. فالأوّلُ يقول «لا وقتَ هذا الشهر»، وهذا يقول
   «ننظر في ملفّك ثمّ ندعوك». ولو جُمعا في مفتاحٍ واحدٍ لَقرأ المدعوُّ وقفا
   لا يعنيه، أو لَفُتح البابُ للجميع يومَ يُطفأ الوقف.

   ═══ ولماذا أُغلق ═══

   كان كلُّ من أرسل الاستمارةَ يجد التقويمَ تحتها مباشرةً فيحجز — فامتلأ
   تقويمُ المُقابِل بمن لم يُقرأ ملفُّه بعد، وهي الشكوى التي وُلد منها الوقفُ
   فوقُ. والوقفُ عالجها شهرا ثمّ عادت يومَ أُطفئ، لأنّه عالج **الوقت** ولا
   يعالج **الترتيب**: من يُقابَل يُنظر في ملفّه أوّلا، ثمّ يُدعى.
   ...
   من أراد يوما أن يفتح الحجزَ لكلّ مقدّمٍ أطفأ هذا السطرَ وحدَه، فتعود
   البطاقةُ كما كانت. ولا يُحذف: ما يُبنى مرّتين يُبنى الثانيةَ ناقصا. */
export const INTERVIEW_BOOKING_INVITE_ONLY = {
  /** أُوقد ٢٤ سبتمبر ٢٠٢٦ — ويُطفأ وحدَه يومَ يُفتح الحجزُ لكلّ مقدّم */
  active: true,
} as const
...
export const APPLICANT_STATUS: Record<string, { label: string; explain: string; tone: 'neutral' | 'progress' | 'good' | 'warn' | 'bad' }> = {
  ...
  submitted: {
    ...
    /* ثلاثةُ نصوصٍ لثلاثة أبواب — و«من الزرّ أدناه» أخطرُها حين يكذب:
       من قرأها ولا زرَّ تحتها ظنّ العطبَ في الصفحة، فأعاد التقديم. ومقدَّمٌ
       لم يُدعَ بعد (`INTERVIEW_BOOKING_INVITE_ONLY`) لا زرَّ تحته. */
    explain: INTERVIEW_BOOKING_PAUSE.active
      ...
      : INTERVIEW_BOOKING_INVITE_ONLY.active
        ? `طلبك كاملٌ عند فريقنا ويُقرأ الآن. فإن رغبنا بلقائك وصلك رابطُ حجز ${TRAINER_INTERVIEW.labelAr} على بريدك — ولا يلزمك شيءٌ حتّى يصل.`
