...
const overrides: Record<string, Partial<PlanEntry>> = {
  ...
  /* متقاعد 2026-08-28 — أثره الوحيد المبرمج عقوبة، لا مطابقة.
     كل شرط يذكر application_readiness في القوالب الستة عشر أُحصي: «عالٍ» وزن
     موجب صغير (0.06–0.12)، و«منخفض» وزن خطر (0.16–0.20). ولا قالب يفضّل خطة
     نظرية لمن استعداده منخفض — لأن لا خطة نظرية في الأكاديمية: كل مسار مشروع
     وواجبات تُراجَع بشريا. فوعده المكتوب («يفضل المسارات العملية أو النظرية»)
     بلا مقابل في المحرك، والخيار الذي يعرضه غير موجود.
     والقياس المضاد للواقع (300 جلسة، بذرة ثابتة): 207 مقاعد، القلب ممكن في
     أربعة منها فقط (1.9٪)، وفي 93 مقعدًا كان أثره خفض الثقة لا رفعها. أي أنه
     يعاقب الصادق في جوابه بمقعد من وقته. وscoreMotivationComponent يسقط عند
     غيابه على 0.5 محايدة، فلا يترك أثرًا في تسجيل المرشحين. */
  'QB-M2-015': { surface: 'retired_b2c', layer21: null, phase: 'none', action: 'retire', stages: [], why_ar: 'أثره المبرمج عقوبة لا مطابقة: لا خطة نظرية يُحوَّل إليها من قال «استعدادي منخفض».', impact_ar: 'كان يخفض الثقة في 93 مقعدًا من 207 ويقلب المسار في أربعة.' },
  ...
  /* أسئلة المواقف الخمسة: العلّة كانت في صياغتها لا في مراحلها.
     أُضيفت (2026-08-26) لتُخرج أربعة مسارات من الخمول، وصيغت كأنها تسأل عن
     وظيفةٍ قائمة: «كيف تُقرَّر كميات الشراء في عملك؟» و«حين تقابل مرشحا
     لوظيفة» و«حين يُطلب منك تدريب» و«حين تختلف نتائج الربع عن الموازنة».
     فطُرحت على طالب جامعي فوجد نفسه أمام سؤال عن عملٍ لا يملكه.

     وجرّبتُ تقييدها بالمراحل، فقِيس الثمن على 10000 جلسة: الإحالة إلى مستشار
     12.55٪ ← 14.94٪، ومتوسط تغطية الحاسمة 93.58٪ ← 89.33٪. أي أن المنع يكلّف
     239 متعلما إضافيا يُحالون إلى مستشار بدل أن يأخذوا توصية — لأن «لم أقترب
     من هذا القرار» جوابٌ يحمل إشارةً حقيقية: يُنزل مسارات العمليات ويرفع
     غيرها. فمنعُ السؤال يحجب الإشارة ولا يحمي أحدا.

     فأُعيدت صياغتها إلى صيغة الخبرة لا صيغة الوظيفة — «ما أقرب وصف لخبرتك
     به؟» — وصار خيارها الأول «لم أقترب من هذا القرار» بدل «لا علاقة لي به».
     يجيب الطالب بصدق، وتصل الإشارة، ولا يُحال أحد إلى مستشار بلا سبب.
     المراحل تبقى 'all' عمدا. */
  ...
  /* أُعيد 2026-08-28. كان قد نُقل إلى ما بعد التوصية لأن english_for_work لم
     تكن مخرج تعلم لأي كيان نشط — ولا دورة تُعلّمها أصلا، فقياسها يكشف فجوة لا
     يوجد ما يسدّها. وأُضيفت C-COMX-106 «الإنجليزية للأعمال» فزال السبب:
     الفجوة المقيسة صارت تجلب مقررا حقيقيا إلى الخطة المركَّبة. ولأن المقرر
     مستقل لا يدخل تسلسل مسار جاهز، لا يصل أحدا إلا حين تظهر فجوته بدليل. */
  'QB-M4-009': { surface: 'b2c', layer21: 'evidence_skill', phase: 'adaptive', action: 'keep', stages: 'all', why_ar: 'الإنجليزية للعمل صارت مهارة يُعلّمها مقرر في الكتالوج — فقياسها يجلب ما يسدّ فجوتها.', impact_ar: 'فجوة مقيسة قوية تُدخل «الإنجليزية للأعمال» في الخطة المركَّبة.' },
