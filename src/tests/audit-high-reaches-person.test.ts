...
/** الدَّينُ المقيس — فعلٌ عالٍ في معالِجٍ لا يُخبِر أحدا
    ...
    كان اثنَين وأربعين يوم كُتب، فصار **أربعةَ عشر**: خمسةٌ سقطت بإصلاح
    الحارس (كان يقرأ أسماءَ المُرسِلين فلا يرى غلافا محلّيّا يُرسل)، وثلاثةٌ
    وعشرون بإشعاراتٍ كُتبت فعلا. */
const SILENT_DEBT: readonly string[] = [
  /* ═══ ما بقي من الحساب ووصولِه: المحوُ والإقلاع ═══

     والمحوُ بابٌ خاصّ: صفُّ `Notification` يُحذف مع صاحبه
     ...
     وذاك ترتيبٌ يستحقّ التزاما مستقلّا يُراجَع وحدَه، لا سطرا يُدسّ في دفعة.

     و`accounts.reset_archive` معه عقدةٌ صغيرة: البريدُ يُعمّى داخل المعاملة
     نفسِها، فيُلتقَط قبلَها أو يُرسَل من الحلقة حيث السببُ في المدى. */
  'admin.user.purge', 'admin.user.purge_with_history', 'admin.users.purge_bulk',
  'accounts.reset_purge', 'accounts.reset_archive',
  /* والإقلاعُ: يقع مرّةً عند بدء الخادم ولا فاعلَ له — ويُخبَر به صاحبُه
     بجرسٍ جمهورُه `staff`، فالافتراضُ `learner` يُنزله في بوّابةٍ لا يفتحها. */
  ...
  /* ═══ الشعبةُ ومن أُسند إليها ═══
     ...
     و`cohort.trainer_update` يدع المدرّبَ يحرّك موعدَ الشعبة ومكانَها ولغتَها
     ولا يبلغ ذلك مسجَّلا واحدا. */
