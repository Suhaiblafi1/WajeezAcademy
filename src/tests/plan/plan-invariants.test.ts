...
describe('عرض السعر — مرحلة الإقناع', () => {
  ...
  /* وسقط معه سطرُ «أوّل عملية شراء» من هذا الصندوق تحديدا — بصريح طلب صاحب
     المنصّة بعد أن رآه ازدحاما في صندوقٍ يكفيه رقمٌ واحد. الكودُ نفسُه يبقى
     حيّا في مكانه الفعليّ: حقل الكوبون عند الدفع (`BuyPanel.tsx`)، لا مُعلَنا هنا. */
  it('«أول عملية شراء» لم تعد تُعلَن في صندوق المسار كاملا', () => {
    expect(PATHWAY).not.toMatch(/FIRST_TIME_PROMO/)
  })
