...
describe('عرض السعر — مرحلة الإقناع', () => {
  ...
  /* انقلب القرار مجددا: خصم الباقة كان جملةً عامة («خصمٌ كبير...») لأن
     الخادم لم يكن يطبّقه — وقد صار يطبّقه فعلا (`cart-pricing.ts`)، فيُعرض
     رقما لا وصفا: السعر الأصلي مشطوبٌ بجانب السعر بعد الخصم. */
  it('خصم الباقة معروضٌ رقميا — الأصلي مشطوبٌ بجانب الحالي', () => {
    expect(PATHWAY).toMatch(/line-through/)
    expect(PATHWAY).toMatch(/بعد خصم الباقة/)
  })

  /* وانقلب القرارُ مرّةً أخرى في «أوّل عملية شراء».

     كان سطرا في صدر الصندوق فحُذف لأنّه ازدحامٌ في موضعٍ يكفيه رقمٌ واحد.
     ثمّ حُذف صندوقُ «بريدٌ مقابل الكود» أسفلَ الصفحة بقرار صاحب المنصّة —
     «لا داعي لهذه الخانة لأنه عند الشراء سيضع ايميله» — فلم يبقَ للكود
     موضعٌ يُعلَن فيه أصلا، وهو أوسعُ العروض: لكلّ مشترٍ في أوّل مرّة.

     فموضعُه الآن **داخل مطويّة الفئات** لا في صدر الصندوق: من فتحها يقرأ
     الثلاثةَ معا — عرضَ أوّل الشراء بكوده المعلَن، وخصمَي الفئتين بإثباتهما.
     فلا ازدحامَ في الصدر، ولا عرضٌ مخفيٌّ بلا موضع. */
  it('«أول عملية شراء» تُعلَن بكودها داخل مطويّة الفئات لا في صدر الصندوق', () => {
    expect(PATHWAY, 'الكودُ غائبٌ عن الصفحة كلِّها').toMatch(/FIRST_TIME_PROMO\.code/)
    /* في المطويّة: بعد سطرِ الدعوة إلى الفئات لا قبلَه */
    const summaryAt = PATHWAY.indexOf('اطّلع على الفئات وتحقّق من أهليتك')
    const promoAt = PATHWAY.indexOf('خصم أول عملية شراء')
    expect(summaryAt, 'مطويّةُ الفئات غائبة').toBeGreaterThan(-1)
    expect(promoAt, 'الكودُ عاد إلى صدر الصندوق قبل المطويّة').toBeGreaterThan(summaryAt)
  })
