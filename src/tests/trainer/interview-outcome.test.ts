...
describe('⑥ ومعجمُ النتيجة للموظّف وحدَه', () => {
  ...
  const ALLOWED = [
    ...
    /* ═══ ومتابعةُ الغياب (٢٣ سبتمبر ٢٠٢٦) — والسؤالُ يُجاب ═══

       «أهذا موظّفٌ يقرّر، أم متقدّمٌ يُقرَّر فيه؟» — هذه وحدةٌ نقيّةٌ تحمل
       نصَّ رسالتَي المتابعة، وتقرؤها شاشةُ الطابور والخادمُ معا. وهي تستورد
       **قيمةَ `NO_SHOW` وحدَها** — مفتاحَ القاعدة، لا `outcomeLabelAr` ولا
       `INTERVIEW_OUTCOMES`. فلا لفظَ حكمٍ يعبُر منها إلى بريدِ أحد: ما يصل
       المتقدّمَ مكتوبٌ فيها بنفسه ويراه الموظّفُ قبل أن يضغط. */
    'src/application/trainer/no-show-followup.ts',
  ]
