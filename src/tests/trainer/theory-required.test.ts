...
describe('ويمنع الاعتمادَ لا الحفظ', () => {
  ...
  it('⚠️ والشاشةُ والخادمُ على أرضيّةٍ واحدةٍ مستوردة — لا رقمَين', () => {
    const ws = code('src/pages/trainer/CohortWorkspace.tsx')
    const svc = code('server/services/cohort-plan.service.ts')
    expect(ws, 'الشاشةُ تكتب رقما بيدها').toContain('MIN_MODULE_BODY')
    expect(svc, 'الخادمُ يكتب رقما بيده').toContain('MIN_MODULE_BODY')
    /* الفحصُ على أرضيّةِ **المتن** وحدَها: `title.trim().length >= 3` حدُّ
       اسمِ الشعبة، رقمٌ محلّيٌّ لا يتقاسمه أحدٌ فلا شأنَ له بهذا. */
    expect(svc, 'أرضيّةُ المتن عادت رقما مكتوبا في الخادم').not.toMatch(/bodyAr[^\n]*length >= \d/)
  })
