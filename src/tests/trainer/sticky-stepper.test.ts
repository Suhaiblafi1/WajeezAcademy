...
describe('① السلّمُ يبقى ظاهرا حتّى يخرج من الشعبة', () => {
  ...
  it('⚠️ ملاصقٌ للسقف: بلا فراغٍ ميّتٍ فوقه، ولا انحناءَ يجلسه في الصفحة', () => {
    ...
    /* ① الفراغُ الميّت: هامشٌ على أخٍ يُخفى بـ`display:none` لا ينطوي معه.
          فلا يُكتب ثابتا — يتبع الضمورَ كما يتبعه من يجاوره. */
    const openTag = head.slice(0, head.indexOf('>') + 1)
    expect(head, 'هامشُ اللسانَين ثابتٌ فلا ينطوي مع ما يُطوى — عشرون بكسلا ميّتة')
      .not.toMatch(/ariaLabel="طورا الشعبة"[\s\S]{0,200}className="mt-5"/)
    expect(head).toMatch(/className=\{compact \? "" : "mt-5"\}/)

    /* ② الالتصاقُ بنيةٌ لا صنفٌ يُمرَّر: `Bar` بلا انحناءٍ وحدُّها السفليُّ
          وحدَه، وتفيض عن حشو حاضنها لتبلغ حافّتَي الإطار. ولا `mb` يفصلها
          عمّا تحتها؟ بل `mb` مشروعةٌ — الفصلُ عن المتن لا عن السقف. */
    expect(WS.slice(WS.indexOf('<Bar'), WS.indexOf('<Bar') + 400),
      'الشريطُ لا يفيض عن حشو حاضنه فلا يبلغ حافّتَي الإطار').toMatch(/-mx-5/)
