...
describe('ورقةُ الطباعة: مقاسٌ ومراتبُ حبرٍ لا كتلةٌ رماديّة', () => {
  ...
  it('وقسمٌ كلُّ ما فيه حقلُ إدخالٍ لا يُطبع أصلا — لا عنوانٌ بلا شيءٍ تحته', () => {
    /* «تقييم الدرس التجريبيّ» ليس فيه إلّا النموذج. فلمّا أُخفيت الحقولُ وحدَها
       بقي عنوانُه معلّقا — وهو العطبُ نفسُه الذي أُصلح في الأقسام المطويّة. */
    expect(ops, 'لا سبيلَ لوسم قسمٍ بأنّه أداةٌ كلُّه').toContain('toolOnly')
    const demo = /<FoldSection[^>]*sec-demo[^>]*>/.exec(ops)?.[0] ?? ''
    expect(demo, 'قسمُ الدرس التجريبيّ غيرُ موسومٍ أداةً — فيُطبع عنوانُه فارغا').toContain('toolOnly')
  })
