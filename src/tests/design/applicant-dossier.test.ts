...
describe('ورقةُ الطباعة: مقاسٌ ومراتبُ حبرٍ لا كتلةٌ رماديّة', () => {
  ...
  it('⚠️ ولا عنوانَ بلا شيءٍ تحته: قسمُ الحقول لا يُطبع، والفارغُ كذلك', () => {
    ...
    /* ١٤ سبتمبر ٢٠٢٦: «الدرسُ التجريبيّ» و«المراجعُ المهنيّة» ذهبا من الشاشة
       بأمر صاحب المنصّة، فلا يُطالَب باستثنائهما من الطباعة. وبقي «العقدُ
       والتوقيع» — حقولٌ خالصةٌ يخرج عنوانُها فارغا لو طُبع. */
    for (const [id, why] of [
...
describe('ورقةُ المقابلة: تُسأل بمحاور التقييم — والتقييمُ يُكتب في الرابط', () => {
  ...
  /* ═══ الشكوى (١٧ سبتمبر ٢٠٢٦): «يجب أن تكون مطويّة — تُعبّي الصفحة» ═══

     ستّةَ عشرَ سؤالا مبسوطةً في شاشةٍ تُقرأ للقرار. والطيُّ ليس إخفاءً:
     العددُ على الغلاف، والفتحُ نقرة. وشرطُ ألّا يقع الطيُّ حيث الأسئلةُ
     هي المقصود: رابطُ القارئ يُفتح في الغرفة لأجلها. */
  it('⚠️ والأسئلةُ مطويّةٌ في شاشة الطلب، مفتوحةٌ في رابط القارئ', () => {
    expect(asks, 'لا طيَّ أصلا — القائمةُ مبسوطةٌ حيثما رُكّبت').toMatch(/<details\b/)
    expect(asks, 'الطيُّ مفروضٌ على الاثنين — لا قرارَ عند المُنادي').toMatch(/<details open=\{defaultOpen\}/)
    /* والغلافُ يقول كم خلفه — وإلّا صار الطيُّ إخفاءً */
    const cover = asks.slice(asks.indexOf('<summary'), asks.indexOf('</summary>'))
    expect(cover, 'لا غلافَ للطيّة أصلا').toContain('أسئلةٌ مقترحةٌ للمقابلة')
    expect(cover, 'غلافٌ لا يقول كم سؤالا خلفه').toContain('{total}')

    /* شاشةُ الطلب لا تطلب الفتحَ — لا في الورقة ولا في الشاشة التي تحتويها */
    expect(sheet, 'ورقةُ المقابلة تفتحها فتعود الشكوى').not.toMatch(/defaultOpen/)
    expect(code('src/pages/admin/TrainerApplications.tsx'), 'الشاشةُ تفتحها فتعود الشكوى')
      .not.toMatch(/<InterviewSheet[^>]*defaultOpen/)

    /* ورابطُ القارئ يفتحها: هي سببُ فتح الصفحة */
    expect(code('src/pages/SharedDossier.tsx'), 'رابطُ الغرفة يفتح على قائمةٍ مطويّة')
      .toMatch(/<InterviewQuestions[^>]*defaultOpen/)
  })
