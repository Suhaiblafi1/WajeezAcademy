  ...
  describe('والمختاراتُ بطاقةٌ وبابان', () => {
    ...
    it('والبطاقاتُ تحمل ما أُلّف لها — لا الاسمَ والمدّةَ وحدَهما (البند ٣٠)', () => {
      /* على **الحقول** لا على العناوين الظاهرة. أوّلُ صياغةٍ فحصت
         ...
         فالفحصُ على `b.p.notFor` وأخواته: مرجعُ حقلٍ لا يوجد إلّا حيث يُقرأ. */
      expect(best, 'العنوانُ الكاملُ مكانَ القصير').toMatch(/b\.p\.shortName/)
      expect(best, 'ذهب «ليس لك إن» — أصدقُ سطرٍ في الكتالوج').toMatch(/b\.p\.notFor/)
      expect(best, 'ذهب المخرَج').toMatch(/b\.p\.output/)
      expect(best, 'ذهب وعدُ الدورة').toMatch(/b\.c\.promise/)
      expect(best, 'ذهبت مهارةُ الدورة').toMatch(/b\.c\.skill/)
    })
    ...
    it('ويُرى أكثرُ من واحد — مساراتٌ ودوراتٌ معا', () => {
      ...
      expect(best, 'بقيّةُ المختارات لا تُقرأ').toMatch(/picks\.slice\(/)
