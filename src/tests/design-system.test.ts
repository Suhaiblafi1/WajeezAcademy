 ...
 *
 * والاتّجاهُ في اتّجاهٍ واحد: **لا يُرفع سقفٌ إلّا بتراجعٍ مُعلَن.** ومن
 * احتاج صيغةً لا يغطّيها السلّم، فالنقصُ في السلّم لا في القاعدة — تُضاف
 * الدرجةُ إلى `ui/Surface.tsx` ويُخفَض السقف.
 *
 * ── ومتى يُرفَع، وكيف ──
 *
 * رُفع مرّةً واحدةً في ٦ سبتمبر (٢٩١ ← ٣٤٨ و٣ ← ٢٦) لثمانيةِ ملفّاتٍ أُرجعت
 * عن الترحيل لتعارضِ أسماء، **وأُعيد في اليوم نفسِه** بعد أن حُلّ التعارض:
 *
 *   · `admin/TrainerOps.tsx` كان فيه مكوّنٌ محلّيٌّ اسمُه `Card` وهو ليس
 *     بطاقةً بل قسمٌ يُطوى — فسُمّي `FoldSection` بما هو.
 *   · وسبعةٌ تستورد `Button` من زرِّ shadcn الموروث — فرُحّلت إلى زرِّ
 *     المنصّة، وحُذف الموروثُ إذ لم يبقَ له مستعمِل.
 *
 * **والرفعُ يُسجَّل ولا يُخفى.** سقفٌ يُرفع بلا سببٍ مكتوبٍ يُبطل الآليّةَ
 * كلَّها: يصير رقما يُطارد الواقعَ بدل أن يقوده. فمن رفعه سمّى ما تراجع عنه
 * ومتى يعود — وهذه الثمانيةُ تُرحَّل بالعين، بأسماءٍ لا تتعارض.
 *
 * ── وحدُّه صريح ──
 *
...
/* ── والسقفُ ينزل مع كلّ ترحيل: ٣٠ ← ٢٨، و١٧٢ ← ١٦٨ في هذه الجولة ──

   رُحِّل في البند ٦ من تقييم لوحة الإدارة **ثلاثةٌ وثلاثون زرًّا** (٦٦ ← ٣٣
   في `pages/admin`، و٢٤٥ ← ٢١٢ في المستودَع): أفعالُ الحفظ والاعتماد والنشر
   والإسناد والحذف — كلُّها صارت `Button` بنبرةٍ من السلّم.

   ── وما بقي ليس دَينا مؤجَّلا، بل صنفٌ آخر ──

   الثلاثةُ والثلاثون الباقيةُ ليست أفعالا تحمل نبرة، بل **عناصرُ بنية**:

     ألسنةٌ وحبّاتُ اختيار  · `Authoring` · `Users` · `CatalogAdmin` · `Support`
     صفٌّ يُنتقى من قائمة    · `Authoring` (الوحدات) · `TrainerApplications`
     أزرارُ ترقيمٍ أيقونيّة  · `AuditLog` — وهي نسخةٌ يدويّةٌ من `ListToolbar`
     خلايا شبكةِ الروبرك    · `TrainerApplications` (١–٥)
     رؤوسُ طيٍّ تُضغط        · `AdminCohorts` · `CohortWizard`

   وترحيلُها يحتاج قرارا لا نسخا: أهي `Button` بنبرتَين؟ أم مكوّنُ «لسان»
   جديدٌ في السلّم؟ فتُترك حتّى يُتّخذ، والسقفُ يمنع نموّها. */
const CEILING = {
