...
describe('وجهةُ النسخ خارجَ الخادم', () => {
  ...
  /* والإذنُ الصريحُ بابُ الإنتاج اليومَ لا حالةً نظريّة.

     قرارُ صاحب المنصّة (١٤ سبتمبر ٢٠٢٦): لا صندوقَ تخزين. فـ`/var/backups/wajeez`
     هو المضبوط، ومعه `ALLOW_LOCAL=yes` — وما يُخرج النسخةَ من الآلة **نسخُ
     Hetzner السحابيّة** التي تلتقط قرصَ الخادم كلَّه، لا `rclone`.

     والمسارُ هنا هو المسارُ المرفوضُ في أوّل الفحوص بعينه — فيُرى الفرقُ
     كلُّه في الإعلان: يُردّ بلا `ALLOW_LOCAL` ويمرّ بها. وذلك ما يفرضه
     الحارس: الإعلانَ، لا القرار. */
  it('والمسارُ المحلّيُّ المأذونُ به صراحةً يمرّ بتحذير — لتجهيزٍ مركوبٍ أو نسخٍ يأخذها غيرُه', () => {
