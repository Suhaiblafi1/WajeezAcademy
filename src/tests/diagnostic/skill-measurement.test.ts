...
describe('ب-٤ تباعد التوثيق عن المحرك — واقعة تُعلَن', () => {
  ...
  it('خمسة أسئلة قياس تقيس مفاتيح ليست مهارات — إجاباتها لا تدخل أي ترشيح', () => {
    const registered = new Set(skillsCatalog.map((s) => s.slug))
    const orphans = [...measurableSkills()].filter((s) => !registered.has(s))
    /* واقعة قائمة في الكتالوج المنشور: يُسأل المتعلم ويُهمَل جوابه.
       الاختبار يحرسها فلا تزيد بلا انتباه. */
    expect(orphans.length).toBeGreaterThan(0)
    for (const slug of orphans) expect(assessSkillSelection([slug]).measured).toBe(1)
  })
})
