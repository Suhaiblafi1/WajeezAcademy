...
export interface CoreCatalogCourse {
  ...
  sequence: number
  title_ar: string
  /** المصطلح المهنيّ المعروف بالإنجليزية.

      كان يُذكر حيث يكون المصطلحُ الإنجليزيُّ أشهرَ من العربيّ في سوق العمل —
      فحمله ستّون من إحدى وثمانين، وبقيت إحدى وعشرون بلا سطرٍ تحت عنوانها
      فاختلفت البطاقاتُ في بنيتها. وقرارُ صاحب المنصّة (١٥ سبتمبر ٢٠٢٦):
      «وترجم كافّة الدورات». فلكلّ دورةٍ في `core-catalog.v2.json` مصطلحُها،
      وحارسُه `src/tests/catalog/course-card-labels.test.ts`.

      والحقلُ يبقى اختياريّا في النوع لا في الكتالوج: لقطةُ API تُبنى من
      قاعدةٍ قد يصلها صفٌّ بلا مصطلح (`snapshot-builder.ts`)، فادّعاءُ
      وجودِه دائما في النوع كذبٌ على ما يصل وقتَ التشغيل. */
...
export interface CoreCatalogModule {
  ...
  /** النشاط التطبيقيّ (ح-٦) — «نشاط/زمن/مخرَج/بديل» ثم «> خطوة: N · نص»؛
      غائب حين لا نشاط مؤلَّف. ولا يُغني عنه `practice_activity_ar`: ذاك
      عبارةٌ واحدةٌ لصفحة الدورة، وهذا خمسون دقيقةً من عمل المتعلّم. */
  module_practice_ar?: string
  /** الروبرك (ح-٧) — «معيار:» وثلاثةُ مستويات؛ غائب حين لا روبرك */
  module_rubric_ar?: string
}
