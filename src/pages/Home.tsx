...
import '../App.css'
/* خطّافُ الظهور عند البلوغ — انتقل إلى `hooks/` حين احتاجته «من نحن» */
    ...
    <section id="top" className="relative overflow-hidden pt-20 pb-2 md:pt-36 md:pb-10">
      ...
      <div className="shell relative text-center">
        ...
        <div className="reveal is-visible mt-11 flex flex-col items-center justify-center">
          ...
          {/* ── السطرُ تحت الدعوة: ما يقع بعد النقر ──

              كان الزرُّ وحدَه بلا سطرٍ يشرحه، فلا يعرف الزائرُ الجديدُ كم
              يُطلب منه ولا إلامَ يُفضي — فيدفع ثمنَ القرار قبل أن يعرف سعرَه.
              والنمطُ قائمٌ في الصفحة نفسِها: دعوةُ الختام تحتها
              «مجاني · بدون حساب · إجاباتك على جهازك» — فهذا إتمامُ نمطٍ لا
              ابتداعُ واحد.

              **والنصُّ يُقاس بوجهة الزرّ لا بما نتمنّاه:** هذه مرساةٌ إلى
              «مؤشر وجيز» في الصفحة نفسِها لا انتقالٌ إلى التشخيص الكامل،
              والمؤشّرُ سؤالان (`mirrorQuestions`) وإجاباتُهما تُحفظ على جهاز
              الزائر وحدَه. فوعدُ «ثلاث دقائق» أو «مسارٌ مفسّر» هنا وعدٌ لا يفي
              به هذا الزرّ — وهي عينُها العلّةُ التي عولجت في دعوة الختام حين
              كانت تعِد بخريطةٍ ثمّ تهبط بالزائر إلى المؤشّر.

              وكلمتان لا ثلاث، بنصّ صاحب المنصّة (١٣ سبتمبر ٢٠٢٦): جُرّب
              «سؤالان سريعان · بلا حساب · إجاباتك على جهازك» فرآه طويلا. */}
          ...
          {/* ── وسطرُ «الفصل القادم» رُفع من الصدر (١٢ سبتمبر ٢٠٢٦) ──

              كان `<UpcomingTermLine prefix="الفصل القادم:" />` هنا، فيُقرأ:
              «الفصل القادم: موسم الخريف 2026 (١ أغسطس — ٣١ أكتوبر) والتسجيل
              مفتوح» — وهي **الجملةُ بعينها** التي أمر صاحبُ المنصّة بحذفها من
              كلّ موضعٍ تظهر فيه.

              ولم تُرفع مع أختها في الكتالوج أوّلَ مرّة لأنّي قصرتُ البحثَ على
              `UpcomingTermBanner` وظننتُ أنّ `UpcomingTermLine` تقول غيرَها —
              وهي تقولها حرفا بحرف متى مُرِّر إليها هذا الـ`prefix`. فالعبرةُ
              أنّ نصَّ الشاشة يُقاس بما **يُصيَّر** لا باسم المكوّن.

              وما بقي من المكوّن في مواضعه الثلاثة الأخرى يُصيَّر «تُفتح في …»
              (البادئةُ الافتراضيّة) داخل سياق السعر — جملةٌ أخرى لم تُطلب. */}
        </div>
      </div>
    </section>
  )
        ...
        <div className="reveal mt-10 flex flex-wrap items-center justify-center gap-10 md:gap-16">
          ...
          {partnerLogos.map((p) => (
            <img
              ...
              loading="lazy"
              decoding="async"
              /* ولا `grayscale` هنا: أحاديّةُ اللون صارت في artwork نفسِه
                 لا في مرشّحٍ فوقه (صاحب المنصّة، ١٥ سبتمبر ٢٠٢٦: «اجعل لوقو
                 الوطن اسود وابيض ليتناسب مع البقيه»). ومرشّحٌ على الأصل لا
                 يجعل الثلاثةَ متّفقة — إنّما يطمس ألوانَ واحدٍ ويترك اثنين.

                 والعتامةُ كاملةٌ كما كانت — شعارٌ بسبعين في المئة شعارٌ باهت.
                 والمقاسُ `h-10 md:h-12` لم يتغيّر: الاتّزانُ نزل إلى الأصول
                 (لوحةٌ واحدةُ الارتفاع للثلاثة) فبقي CSS كما هو. */
              className="partner-logo h-10 w-auto transition md:h-12"
            />
          ))}
        </div>
        {/* «وجيز مهارات بالأرقام» بين الشعارَين — تحت «شركاؤنا» وفوق «مؤسسات
            وثقت بمنظومة وجيز»، بقرار صاحب المنصّة (٧ سبتمبر ٢٠٢٦). */}
        <TrustMetricsBar nested />
...
/* ───────────────── page ─────────────────
   الترتيب المعتمد: بطل واضح ← مؤشر وجيز ← كيف تعمل الرحلة ← شركاء/إعلام
   (وفيه: الشعارات ← أرقام وجيز مهارات ← مؤسسات المنظومة) ← مخرجات التعلم
   ← مسارات ودورات مميزة ← قصص حقيقية
