    ...
    <PortalLayout title="إشعاراتي">
      <div className="mb-4 flex items-center gap-3">
        ...
        {rows.some((n) => n.status !== "read") && (
          <button onClick={() => void markAll()} className="flex cursor-pointer items-center gap-1.5 rounded-full border border-[#38A7B4]/40 px-4 py-2 text-xs font-bold text-[#6EC7D1] hover:bg-[#38A7B4]/10">
      ...
      {loading ? (
        ...
        <div className="grid place-items-center rounded-3xl border border-white/10 bg-white/[0.02] py-20 text-center">
          ...
          <p className="mt-4 text-sm text-white/50">لا إشعارات بعد — قرارات التسجيل والجلسات والتصحيح تصل هنا فورا.</p>
        </div>
      ) : (
          ...
          {rows.map((n) => (
            <button key={n.id} onClick={() => void markRead(n)}
              className={`w-full cursor-pointer rounded-2xl border p-4 text-right transition ${
                n.status === "read" ? "border-white/5 bg-white/[0.01] text-white/50" : "border-[#38A7B4]/25 bg-[#38A7B4]/5 hover:border-[#38A7B4]/50"
              }`}>
              <p className="flex items-center gap-2 text-sm font-black">
                {n.status !== "read" && <span className="h-2 w-2 shrink-0 rounded-full bg-[#FABC05]" />}
                {n.title}
              </p>
