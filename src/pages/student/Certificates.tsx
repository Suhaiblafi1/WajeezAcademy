  ...
  if (!growth) {
    return (
      <Link
        ...
        className="mt-3 flex items-center gap-2 rounded-2xl border border-teal/25 bg-teal-ink/[0.06] px-3 py-2 text-fine font-bold text-teal-light-ink transition hover:border-teal/50"
      >
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          {rows.map((c) => (
            <article key={c.id} className={`rounded-3xl border p-6 ${c.status === "revoked" ? "border-red-500/30 bg-red-500/5" : "border-white/10 bg-white/[0.03]"}`}>
              ...
              {c.revocation && <p className="mt-2 rounded-xl border border-red-500/30 bg-paper/20 p-2 text-fine text-red-300">سبب الإلغاء: {c.revocation.reason}</p>}
