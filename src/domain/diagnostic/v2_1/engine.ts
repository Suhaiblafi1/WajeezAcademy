...
export function scoreAdaptiveQuestionV21(
  ...
  /* ── ولا فصلَ بالمهارةِ قبل أن يُعرف المجال (2026-09-16) ──

     من قال «غير متأكّد» في هدفِه واحتياجِه معا لا مجالَ له بعد، فالمرشّحون
     المتصدّرون عنده ليسوا مرشّحيه — هم ما تصادف أن تصدّر بلا إشارةٍ منه. وقياسُ
     مهارةٍ لتفصل بين هؤلاء يستهلك مقعدا لا يغيّر شيئا حقيقيّا، ويزيح السؤالَ
     الذي كان سيقول ما يريده أصلا.

     والحادثة: لمّا صار في الفضاء ستُّ كياناتٍ جديدة، ضاق الهامشُ بين المتصدّرين
     (margin < 0.15) فاشتعل هذا البند بوزنِه ٠٫٩ وغلب الاستكشافَ (٠٫٦). فخرجت
     رحلةُ «غير محسوم» من عشرةِ أسئلةٍ تنتهي بمسارٍ بثقة ٦٠٪ إلى أربعةَ عشرَ
     سؤالا تنتهي بـ**لا شيء** بثقة ٣٦٪ — كلُّها أسئلةُ مهارةٍ ولا سؤالَ ميلٍ
     واحد. أي أنّ أشدَّ الناسِ حاجةً إلى التشخيص صار أقلَّهم حظّا منه.

     والقيدُ هنا نفسُ القيدِ الذي وضعه المؤلّفون على `minimumEvidence` أدناه
     (`!unsureExplore`) — بُني هناك ولم يُبنَ هنا. وهو مرفوعٌ بمجرّد أن يتّضح
     المجالُ من أسئلةِ الميل: عندها يصير الفصلُ بالمهارةِ فصلا حقيقيّا.
     يحرسه src/tests/diagnostic/v2_1/explorer-seat.test.ts. */
  const domainUnknownExplorer =
  ...
  const evidenceSeparation =
    plan.layer21 === 'evidence_skill' && sepVal > 0 && margin < 0.15 && !domainUnknownExplorer ? sepVal : 0
