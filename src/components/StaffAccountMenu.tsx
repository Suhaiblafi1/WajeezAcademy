    ...
    <div className="relative">
      <button
        ...
        className={`flex h-10 min-w-0 cursor-pointer items-center gap-2 rounded-full border px-2.5 text-xs font-bold transition ${
          open ? 'border-gold/50 bg-gold/10 text-gold-ink' : 'border-white/12 text-white/65 hover:border-white/30 hover:text-white'
        }`}
      >
        ...
        <>
          ...
          <div role="menu" className="absolute left-0 top-12 z-50 w-72 rounded-2xl border border-white/10 bg-surface p-2 shadow-2xl">
            <div className="px-3 pb-3 pt-2">
              ...
              <p dir="ltr" className="mt-0.5 truncate text-right text-[11px] text-white/45">{user?.email ?? '—'}</p>
              {roles.length > 0 && (
            ...
            <div className="border-t border-white/10 pt-1.5">
              ...
              <Link
                to="/student/account"
                onClick={() => setOpen(false)}
                className="flex items-center gap-2.5 rounded-xl px-3 py-2.5 text-xs font-bold text-white/65 transition hover:bg-white/[0.04] hover:text-white"
              >
