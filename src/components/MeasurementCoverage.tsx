...
function Bar({ value }: { value: number }) {
  ...
  return (
    <span className="flex items-center gap-2">
      ...
      <span dir="ltr" className="tabular-nums text-fine text-foreground">{pct}%</span>
    </span>
  );
}
...
export default function MeasurementCoverage({ className = "" }: { className?: string }) {
  ...
  return (
    <section className={className}>
      ...
      <p className="mt-2 max-w-3xl text-read leading-6 text-foreground">{coverageHeadlineAr(r)}</p>
        ...
        <Card tone="warn" className="mt-4">
          ...
          <ul className="mt-3 space-y-2">
            {topGaps.slice(0, 3).map((g) => (
              <Inset as="li" key={g.slug} className="flex flex-wrap items-center gap-2 px-3 py-2 text-fine">
      ...
      <div className="mt-5 overflow-x-auto">
        <table className="w-full min-w-[38rem] text-right text-xs">
          ...
          <thead>
            <tr className="text-fine text-muted-foreground">
            ...
            {r.pathways.map((p) => (
              <tr key={p.pathwayId} className="border-t border-white/8">
                <td className="py-2.5 align-top">
                  ...
                  <span dir="ltr" className="ms-2 font-mono text-fine text-muted-foreground">{p.pathwayId}</span>
                </td>
                ...
                <td className="py-2.5 align-top text-fine leading-5 text-foreground">{p.costAr}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      <button onClick={() => setOpenGaps(!openGaps)}
        className="mt-4 flex cursor-pointer items-center gap-1.5 text-fine font-bold text-teal-light-ink hover:text-foreground">
        ...
        <ul className="mt-3 flex flex-wrap gap-2">
          {shownGaps.map((g) => (
            <li key={g.slug} title={`المسارات: ${g.pathwayIds.join("، ")}`}
              className="rounded-full border border-white/12 bg-paper/20 px-3 py-1 text-fine text-foreground">
              {g.nameAr}
          ...
          {openOrphans && (
            <ul className="mt-3 space-y-1.5">
              {r.orphanQuestions.map((q) => (
                <li key={q.questionId} className="flex flex-wrap items-center gap-2 text-read leading-6">
      ...
      <p className="mt-4 flex items-start gap-2 text-read leading-6 text-muted-foreground">
