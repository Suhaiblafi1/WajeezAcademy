...
export default function MyDeadlines({ className = "" }: { className?: string }) {
  ...
  if (data.items.length === 0) {
    return (
      <p className={`flex flex-wrap items-center gap-2 rounded-2xl border border-white/10 bg-white/[0.02] px-4 py-2.5 text-read leading-6 text-muted-foreground ${className}`.trim()}>
  ...
  return (
    ...
    >
      ...
      {data.retrievalDue > 0 && (
        <p className="mt-4 flex flex-wrap items-center gap-2 text-read text-muted-foreground">
