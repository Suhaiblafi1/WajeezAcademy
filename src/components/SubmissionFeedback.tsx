...
export default function SubmissionFeedback({
  ...
  return (
    <div className={`space-y-2.5 ${className}`.trim()}>
        ...
        <Card className="bg-paper/20 p-3.5">
          ...
          <ul className="mt-2.5 space-y-1.5">
            {rubric.map((c) => {
              ...
              return (
                <li key={c.id} className="grid grid-cols-[1fr_auto] items-center gap-3 text-read">
      ...
      {revision && (
        <Inset as="p" tone="warn" className="flex items-start gap-2 px-3.5 py-2.5 text-read leading-6 text-foreground">
      ...
      {submission.reviewNote && (
        <Card tone="accent" className="bg-teal-ink/[0.06] p-3.5">
          ...
          <p className="mt-1.5 text-read leading-6 text-foreground">{submission.reviewNote}</p>
        </Card>
      )}

      {submission.feedback.map((f, i) => (
        <Card key={i} className="p-3.5">
          <p className="flex flex-wrap items-center gap-2 text-read font-black text-foreground">
          ...
          <p className="mt-1.5 whitespace-pre-line text-read leading-6 text-foreground">{f.body}</p>
        </Card>
      ))}
    </div>
  );
