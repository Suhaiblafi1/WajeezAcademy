    ...
    <div className="space-y-4">
      ...
      <Panel as="section" tone="warn" className="sm:p-5">
        ...
        <div className="mt-4 flex flex-wrap items-center gap-3 border-t border-gold/20 pt-3.5">
          <p className="min-w-0 flex-1 text-read leading-5 text-muted-foreground">
      ...
      {completion && (
        <Card as="section" className="p-3.5">
          <div className="flex flex-wrap items-center justify-between gap-x-3 gap-y-1">
            <p className="text-read leading-5 font-black text-foreground">
              أنجزت <span className="tabular-nums">{completion.done}</span> من{" "}
              <span className="tabular-nums">{completion.total}</span> دورات المسار
            </p>
            <p className="text-read tabular-nums text-muted-foreground">{completion.percent}٪</p>
          </div>
...
function RequestCard({
  ...
  return (
    <Card as="section" className="flex flex-col">
      ...
      <p className="mt-1.5 text-read leading-6 text-muted-foreground">{bodyAr}</p>
      ...
      {request || sent ? (
        <Inset className="mt-3">
          ...
          {request?.decisionAr && <p className="mt-2 text-read leading-5 text-muted-foreground">{request.decisionAr}</p>}
        </Inset>
      ) : loading ? (
        <p className="mt-3 flex items-center gap-2 text-read text-muted-foreground">
          <Loader2 className="h-3 w-3 animate-spin" /> نقرأ إنجازك…
        </p>
      ) : !eligible ? (
        <ul className="mt-3 space-y-1 border-r-2 border-white/10 ps-3">
          {reasonsAr.map((r) => (
            <li key={r} className="text-read leading-5 text-muted-foreground">{r}</li>
          ))}
        </ul>
      ) : (
        <div className="mt-3">
      ...
      {error && <p className="mt-2 text-read font-bold text-gold-ink">{error}</p>}
    </Card>
  );
}
