  ...
  return (
    <Card className="mb-5">
      ...
      >
        ...
        <span className="shrink-0 text-micro font-bold text-muted-foreground">{options.length} بديل</span>
      </button>
      {open && (
        <>
              ...
              <li key={o.id}>
                ...
                >
                  ...
                  {busy ? (
                    ...
                    <span className="shrink-0 text-micro font-black text-teal-light-ink">انقلني</span>
                  )}
                </button>
              </li>
            ))}
          </ul>
          {error && <p className="mt-2 text-micro leading-5 text-red-300">{error}</p>}
        </>
      )}
    </Card>
  );
