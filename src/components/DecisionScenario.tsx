  ...
  return (
    ...
    >
      <div className="flex flex-wrap items-baseline justify-between gap-2">
        ...
        <p className="text-fine text-muted-foreground">قرارك يفتح مسارا — والمسار يظهر أثره</p>
      </div>
      ...
      <p className="mt-2 text-fine leading-6 text-muted-foreground">{NO_GRADE_NOTE}</p>
        ...
        <ol className="mt-5 space-y-2">
          {taken.map((t, i) => (
            <Card as="li" key={`${t.nodeTitle}-${i}`} className="px-4 py-3">
              ...
              {t.effectAr && <p className="mt-1.5 ps-6 text-fine leading-6 text-foreground">{t.effectAr}</p>}
            </Card>
          ))}
        </ol>
      )}
      ...
      <Card className="mt-5 bg-paper/20">
        <p className="flex items-center gap-1.5 text-fine font-bold text-muted-foreground">
