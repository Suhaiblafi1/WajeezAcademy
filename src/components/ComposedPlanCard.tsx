    ...
    <Panel as="section" tone="accent" className="mt-8 md:p-7" aria-labelledby="cpc-title">
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="min-w-0 flex-1">
                ...
                {(() => {
                  ...
                  return (
                    <p className="mt-1.5 flex items-start gap-1.5 text-read leading-relaxed text-teal-light-ink">
      ...
      {courseList && (plan.deferred?.length ?? 0) > 0 && (
        <Card className="mt-4 border-dashed p-3.5 md:p-4">
          <p className="text-read leading-5 font-black text-muted-foreground">وهذان لمرحلتك التالية — لا لهذه الخطة</p>
          ...
          <ul className="mt-2.5 space-y-1.5">
            {plan.deferred!.map((d) => (
              <li key={d.courseId} className="flex flex-wrap items-center gap-x-2.5 gap-y-1 text-read leading-5 text-muted-foreground">
      ...
      {plan.uncoveredGaps.length > 0 && (
        <Inset as="p" className="mt-4 flex items-start gap-2 px-4 py-3 text-read leading-relaxed text-muted-foreground">
