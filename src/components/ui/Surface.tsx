...
const DEPTH = {
  ...
  /* ═══ الشريطُ — سطحٌ يُلحَم بحافّةٍ لا يجلس في الصفحة ═══

     أُضيف في ١٧ سبتمبر ٢٠٢٦ لشكوى صاحب المنصّة: «اجعلها ملاصقةً للسقف عند
     النزول للأسفل». والالتصاقُ لا يُشترى بأصنافٍ تُمرَّر إلى `Panel` — قاعدةُ
     هذا الملفّ أن لا حدَّ ولا انحناءَ ولا أرضيّةَ في `className`، وإلّا عادت
     الصيغُ السبعُمئة من الباب الخلفيّ.

     فالفرقُ بنيويٌّ لا زخرفيّ: **البطاقةُ تجلس في الصفحة فلها انحناءٌ من
     أربع جهاتٍ وحدٌّ محيط، والشريطُ يُلحَم بحافّةٍ فلا انحناءَ له وحدُّه
     السفليُّ وحدَه** — هو الخطُّ الذي يفصله عمّا تحته. والحشوُ الأفقيُّ
     متروكٌ للمُنادي لأنّ الشريطَ يفيض عن حاضنه ليبلغ حافّتَي الإطار. */
  bar: 'rounded-none border-0 border-b py-4',
} as const
...
/** شريطٌ ملحومٌ بحافّة — لا درجةَ عمقٍ بل شكلُ جلوس. ويُعطى `tone="solid"`
    متى مرّ تحته متنٌ: القياسُ لا يجوز أن يتبع ما يمرّ خلف الشريط. */
