  ...
  return (
    <div className="mt-3 border-t border-white/8 pt-3">
      <button
        ...
        aria-expanded={open}
        className="flex cursor-pointer items-center gap-1.5 text-fine font-bold text-muted-foreground transition hover:text-foreground"
      >
      ...
      {open && (
        <div className="mt-2.5">
          ...
          {error && <p role="alert" className="text-read font-bold text-red-300">{error}</p>}
          {data && data.events.length === 0 && (
            <p className="text-read text-muted-foreground">لا أثرَ مسجّلا على هذا العنصر بعد.</p>
          )}
          {data && data.events.length > 0 && (
              ...
              {data.events.map((e) => (
                <Inset as="li" key={e.id} className="px-3 py-2">
                  <div className="flex flex-wrap items-baseline gap-x-2 gap-y-1">
                    <span className="text-fine font-black text-foreground">{e.actionAr}</span>
                    <span className="text-fine text-muted-foreground">— {e.actorAr}</span>
                    <span className="text-fine tabular-nums text-muted-foreground">{fmtDateTime(new Date(e.createdAt))}</span>
                  </div>
                  {e.reason && <p className="mt-1 text-read leading-5 text-muted-foreground">السبب: {e.reason}</p>}
                  {e.changed.length > 0 && (
                    <p className="mt-1 text-read text-muted-foreground">تغيّر: {e.changed.join("، ")}</p>
                  )}
                </Inset>
              ))}
              {data.total > data.events.length && (
                <li className="text-read text-muted-foreground">
