  ...
  if (!view.revealed) {
    return (
      <Card as="section">
        ...
        <p className="mt-3 flex items-start gap-2.5 text-read leading-6 text-muted-foreground">
  ...
  return (
    <Card as="section">
      <div className="flex flex-wrap items-baseline justify-between gap-3">
        <h2 className="text-sm font-black">{titleAr}</h2>
        <span className="text-fine text-muted-foreground">{view.count} تقييما</span>
      </div>
      ...
      <div className="mt-5 space-y-1.5">
        {[...view.distribution].reverse().map((d) => (
          <div key={d.score} className="flex items-center gap-2 text-fine">
        ...
        <div className="mt-6 space-y-2.5">
          ...
          {view.comments.map((c, i) => (
            <blockquote key={i} className="rounded-xl border border-white/[0.07] bg-paper/20 px-4 py-3">
              <span className="mb-1 block text-fine font-bold text-gold">{c.score} ★</span>
              <p className="text-read leading-6 text-foreground">{c.commentAr}</p>
            </blockquote>
          ))}
        </div>
      )}
