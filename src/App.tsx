...
import RequireRole, { ADMIN_ROLES, ADVISOR_ROLES, APPLICANT_ROLES, LEARNER_ROLES, TRAINER_ROLES } from './components/RequireRole'
...
/* أعلى الصفحة عند كل تنقّل — وبإلغاء استعادة المتصفّح لا بمنافستها.

   شكا صاحبُ المنصّة مرّتين أنّ صفحة التشخيص «مقصوصة من الأعلى»: العنوانُ
   فوق حافّة الشاشة والنصُّ يبدأ من منتصف جملة. ولم تظهر الحالةُ في متصفّحٍ
   محلّيّ سريع قطّ — وظهرت عنده على 4G. وهذا يسمّي السبب:

   `history.scrollRestoration` قيمتُه `auto`، فالمتصفّح يحفظ موضعَ اللفّ
   ويعيده **بعد أن يبلغ المستندُ ارتفاعا يسمح به**. وصفحاتُنا محمّلةٌ كسولا
   (lazy)، فترتيبُ الأحداث على شبكةٍ بطيئة: يقع التنقّل ← يُنفَّذ scrollTo(0,0)
   على مستندٍ قصير (شاشةُ الانتظار) ← تصل الحزمة ← يطول المستند ← **فيستعيد
   المتصفّحُ موضعَه المحفوظ الآن**، بعد أن مضى أمرُنا. فيرى الزائرُ صفحةً
   مقصوصة، ولا يقع ذلك حيث تصل الحزمةُ قبل الرسم الأوّل.

   ومنافسةُ المتصفّح بتأخيرٍ أو بمؤقّت مقامرةٌ على شبكةٍ لا نعرفها. و`manual`
   تُلغي الاستعادةَ من أصلها، فيبقى أمرُنا وحدَه — وهو ما تفعله التطبيقاتُ
   أحاديةُ الصفحة عادةً، لأنّ الاستعادةَ صُمّمت لمستنداتٍ تُحمَّل كاملة. */
      ...
      <main id="main-content" tabIndex={-1}>
        ...
        <Routes>
          ...
          {/* بوابةُ المتعلّم — محروسةٌ كسائر البوابات.
              ...
              كانت مساراتها كلُّها خارج كلّ حارس، ومعها «تقييمي» للمدرّب
              و«مراجعةُ التقييمات» للإدارة. والخادمُ يحرس بياناتِها (مسارُ
              بوابة التعلّم كلُّه بحارس صلاحيّة)، فلم تكن ثغرةَ بيانات — لكنّ
              من يفتحها بدورٍ آخر كان يرى الهيكلَ ثمّ أخطاءَ ٤٠٣، بدل أن
              يُوجَّه إلى بوابته. */}
