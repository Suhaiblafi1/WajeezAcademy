...
| T2 | The global 300 req/min per-IP limit includes `/api/auth/me`; a fast user (or a shared office IP) sees «تعذّر التحقّق من صلاحيّاتك» instead of "busy, retry" | `RATE_LIMIT_MAX` knob added; exemption of `/api/auth/me` and a clearer guard message remain in Phase A1. Note the correction under T9: until `trustProxy` was set, this 300/min was **not** per IP at all but a single global bucket, so the ceiling was reached far sooner than the number suggests |
...
| T9 | Login is limited to **10 attempts per 5 minutes per IP**; the journey script itself was locked out on its 11th login. A classroom behind one NAT hits this on day one | ✅ **fixed on this branch** (task 17, owner decision of 4 Sep: 40 and 25). Three ceilings replace one: 5 per (email + IP), 40 per IP, 25 per email — see doc 05 §18. Implementing it surfaced a worse defect: **`trustProxy` was unset, so `request.ip` was the socket address, not the visitor's.** Behind any proxy — the Vercel function, or Caddy after the migration — every per-IP bucket was in fact **one shared bucket for all visitors**, which made the route cap of 10/5 min global: ten *successful* logins by ten different people in five minutes locked the eleventh. Now `trustProxy: 1` (with the deployment condition that the app port is never exposed directly), the login route cap is `LOGIN_RATE_MAX` (200/5 min, a flood guard rather than a guessing guard), and there is an index on `LoginAttempt(ip, createdAt)` because the per-network count is on the login path. Seven tests in `auth-throttle.test.ts`, the first of which asserts that **the innocent are not locked out** |
...
## 0.8 · "Is the system healthy?", "who changed this?", and the worker (stage 2F)

| # | What was true | Now |
|---|---|---|
| S4 | **"Is the system healthy?" had no answer outside the server logs** — and the server had no logs at all before this branch. When a learner said no reminder arrived, nothing told the owner that 43 notifications sat queued because no worker exists | `/admin/system-health` (super-admin only, `settings.manage`): five groups, sixteen items, every one computed from current database state at page load — deferred promises (queued/failed notifications with the age of the oldest, overdue advisor follow-ups, scheduled publishes past due), money and providers (payment driver, email channel, unprocessed payment webhooks, pending refunds), login and access (failed logins in 24h, stale sessions, expired unused invites), RBAC drift against `ROLE_PERMISSIONS`, and storage (uploads flag, documents held in the DB, last applied migration). Each item states **what it means for the owner**, not a bare number: "1 — oldest 5 hours ago" carries "every row here is a promise to notify that never reached its owner, because the background worker is not running yet" |
| S5 | **"Who changed this cohort?" was answerable only by leaving the screen**, knowing the entity type and its 36-character id, and filtering the global log. And that log showed raw keys — `cohort.session.add` in Latin monospace — the same defect fixed on the admin dashboard (T5) and left here | `GET /api/admin/audit/entity/:type/:id` plus a collapsible timeline in the cohort card and the account card (`audit.view`; hidden, not refused, for those without it). Action names are Arabic from one compositional lexicon (`src/application/audit/labels.ts`) shared by both screens — 130 action keys covered, and a test fails the day a new one is added without a name. The timeline names the actor ("النظام" when no human acted), shows the written reason, and lists **which fields changed** rather than dumping the payload |
| S6 | **Assigning roles wrote no audit event at all.** The single highest-authority action on the platform — the one that makes an account a super admin, or strips a learner's portal — left no trace, while everything below it was recorded: suspension, archiving, even granting one permission by exception | `roles.set` is recorded with the sorted role list before and after, so the answer is "became a super admin", not "roles were changed". Found by the coverage test in this batch, not by reading |
| S7 | Two mandatory reasons were **collected and then hidden**: account archiving and certificate revocation wrote the reason into `meta`, while the screen and the filters read the dedicated `reason` column. Requiring a reason and then not showing it defeats the requirement | both now write `reason` |
| A2 | **No background job runner** — every "we will notify you" is false | **written and tested, not running.** `server/worker/` holds five jobs (dispatch the notification queue, session reminders at 24h and 1h, cohort status by date, scheduled publishing, cleanup of rows expired over 30 days) behind `WORKER_ENABLED=on`, with a `pg_try_advisory_lock` so two copies cannot double-send. Every job is **safe to repeat** — the guard is a question to the database, never stored state — and each is capped per cycle so a restart after an outage cannot fire a thousand notifications at once. 13 tests, each running its job twice and asserting the second run changes nothing. It stays off until the long-running server exists (task 54); Vercel's function sleeps between requests, which is why the queue was never drained |

**Why no queue library.** pg-boss was in the original design (doc 02 §0). All five jobs are idempotent and driven by state rather than messages, so a durable queue would add a third moving part to operate, monitor and migrate for no behaviour we need. The note is in the runner's header: if a job appears that cannot be safely repeated — sending money, say — that is when to revisit, not before.
...
| B7 | **Per-entity audit timeline tabs + coverage test** (doc 02 §7) | Admins see "who changed this cohort" where they work | `/admin/audit` global only | ✅ **done** (stage 2F): timeline in the cohort and account cards, Arabic action names, coverage test — which found S6 |
...
| Global rate limit 300/min per IP; tighter on identity routes. Public POST endpoints (`/api/leads/discount-email`, `/api/v1/trainer-applications`, `/api/events`, `/api/path-drafts`) rely on the global limit only | `app.ts:82`, route scan | Medium — add per-route limits and a honeypot field to public forms. **Partly done (4 Sep):** `/api/auth/register` now has purpose-built ceilings keyed by network — 40 accounts/hour and 100/day for accounts actually created, and 10 per 15 min for attempts that bounced with "email already registered", which is the enumeration channel the old 10-per-5-min cap was throttling only by accident. Admin-created accounts are deliberately exempt (`registerSelf` vs `register`). **Done (4 Sep):** a honeypot field (`website`) on the three public human-typed forms — register, password reset, trainer application — rejected with 400 `form_rejected` and a message that tells a human what to do, deliberately not silently swallowed (dropping a real trainer's application unheard is worse than a bot learning). Its limit is stated in the code and in doc 05 §20: it catches bots that drive a real browser, not the commoner kind that POSTs JSON straight at the route, so `/api/path-drafts` also gained the per-route cap it lacked (30 per 15 min; it was on the global 300/min alone). `/api/leads/discount-email` no longer exists — `main` removed it and a test guards its absence |
