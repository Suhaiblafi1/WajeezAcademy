...
export async function buildTemplateDb(): Promise<void> {
  ...
  /* ═══ قرارُ تشغيلٍ لا يُورَّث إلى القالب ═══

     ترحيلُ `registration_season_gate` يكتب صفًّا في `SystemSetting` يُغلق
     بابَ التسجيل للمنصّة كلِّها — وهو قرارُ صاحب المنصّة لبيئته، لا حقيقةٌ
     عن الشيفرة. و`migrate deploy` يجري هنا كما يجري في الإنتاج، فلو بقي
     الصفُّ لَردَّت **كلُّ** اختبارات الشراء والدفع والتسجيل بـ«لم يفتح باب
     التسجيل» — وهي تختبر الشراءَ لا سياسةَ فتحه.

     فالقالبُ منصّةٌ لم يُقرَّر فيها شيءٌ بعد، والافتراضُ عند غياب الصفّ
     «مفتوح» (العلّةُ في رأس `registration-window.ts`). ومن يختبر القفلَ
     نفسَه يكتب صفَّه بيده — وهو ما يفعله `season-gate.test.ts`.

     ⚠ ولا يُعمَّم هذا على كلِّ صفٍّ يبذره ترحيل: ما كان **بنيةً** (صلاحيّاتٌ
     وأدوار) يبقى، وما كان **سياسةَ تشغيلٍ** يُمحى. والفرقُ أنّ الأوّل تعتمد
     عليه الشيفرةُ لتعمل، والثاني يقرّره إنسانٌ ويبدّله متى شاء. */
  await prisma.systemSetting.deleteMany({ where: { key: 'registration.season' } })
