 ...
 * `VERCEL_*` وحده، فلمّا انتقلت المنصّة عن Vercel اختفى المتغيّر وامتنع
...
describe('/api/version', () => {
  ...
  it('يقرأ البصمة باسمٍ محايدٍ للمضيف — لا VERCEL وحده', async () => {
    ...
    const v = await version()
    expect(v.الكود.الالتزام, 'اسمٌ محايدٌ لم يُقرأ — وهذا هو عطبُ ترك Vercel بعينه').toBe('facade9')
