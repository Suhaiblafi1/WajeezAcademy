...
export class PublishingService {
  ...
  async validateDrafts(): Promise<{ ok: boolean; errors: string[] }> {
    ...
    for (const c of approved.courses) {
      ...
      /* ═══ ودورةٌ بلا مهاراتٍ تُنشَر ولا يراها أحد (ك-٣) ═══

         كان التعليقُ فوق هذه الحلقة يقول «ومهاراتها موجودة»، والحلقةُ تفحص
         أنّ كلَّ رابطةٍ تُحلّ إلى مهارةٍ حقيقيّة. **والصفرُ يمرّ**: حلقةٌ على
         مصفوفةٍ فارغةٍ لا تدور، فلا خطأ.

         والوصولُ إلى التشخيص شرطُه `status === 'published'` وحدَه — لا علَمٌ
         ولا رابطةُ مهارة. فدورةٌ تُنشَر بلا مهارةٍ **تدخل الكتالوجَ ولا
         يرشّحها مطابقُ المهارات لأحدٍ أبدا**: حيّةٌ في القائمة، ميّتةٌ في
         المحرّك. ولا شكوى تصل، لأنّ من لم يُرشَّح له شيءٌ لا يعرف أنّه فاته.

         والحاجزُ على النشر لا على الاعتماد — كما في د-١: الاعتمادُ حكمٌ على
         أنّها دورةٌ حقّا، وربطُ المهارات تهيئتُها للمتعلّمين. وقلبُ الترتيب
         يمنع الإدارةَ أن تقول «نعم» قبل أن يتمّ العمل.

         والقالبُ تحتَه يفحص هذا بعينه منذ أوّل يوم (`بلا دورات`) — فالنقصُ
         كان في الدورة وحدَها. */
      if (links.length === 0) errors.push(`course ${c.id}: بلا مهارات — لن يرشّحها التشخيص لأحد`)
      for (const l of links) {
