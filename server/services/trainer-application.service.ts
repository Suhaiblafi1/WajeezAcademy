...
/** وجهاتُ حالةٍ حيّة: كلُّ حيّةٍ سواها، والاعتمادُ والردُّ والانسحاب.
    ...
    و`submitted` تخرج من **الوجهات** وتبقى في المصادر: معناها «قُدِّم ولم
    يُقرأ بعد»، فالرجوعُ إليها بعد قراءةٍ يجعل الحالةَ تكذب على من يقرؤها —
    ويُخرج الطلبَ في طابور «لم يُقرأ» وقد قُرئ. ولا يُفقَد بذلك بابٌ: لا قرارَ
    في `DECISIONS` وجهتُه `submitted` أصلا، ومن أُريد ردُّه إلى أوّل الطابور
    فبابُه «بدء المراجعة». */
const openTargets = (from: TrainerStatus): TrainerStatus[] => [
  ...REVIEW_OPEN_STATUSES.filter((s) => s !== from && s !== 'submitted'),
...
export class TrainerApplicationService {
    ...
    const mail = await sendDirectEmail(this.prisma, {
      ...
      ...renderMail({
        ...
        blocks: [
          ...
          /* الرابطُ على كلمةٍ لا عنوانا خامّا في وسط الجملة (١٧ سبتمبر ٢٠٢٦).
             والنصُّ الخامُّ يحمل العنوانَ بين قوسين — يبنيه `mail-template`. */
          { kind: 'list', items: [
            [
              'بالدخول إلى ', { text: 'صفحة الدخول', href: `${site}/auth` },
              ' ببريدك هذا وكلمة المرور التي اختَرتها عند التقديم.',
            ],
            [
              'أو من ', { text: 'صفحة الانضمام', href: `${site}/join-trainer` },
              ' بإدخال بريدك.',
            ],
          ] },
  ...
  /* ─────────── الحذف النهائيّ — لا التعطيل ───────────
     ...
     والمقابلات والمراجع وسجلّ الحالات والدعوات والتخصّصات)، والملفُّ يُحذف
     بيدٍ في المعاملة نفسِها (`CohortTrainer` مرجعٌ لازمٌ بلا تتالٍ).

     ═══ والحرّاسُ أربعةٌ، وكلٌّ عن شيءٍ غيرِ سجلّنا ═══

     ١) **ولا حذفَ بلا سبب.** يُكتب في سجلّ التدقيق **قبل** الحذف — فيبقى
        الأثر بعد أن يذهب الصفّ.
     ٢) **ولا يُمحى من بيد متعلّمٍ شهادةٌ منه.** رقمُها معلَنٌ ويُتحقَّق منه
        برابط، فمحوُ شعبته يجعل الرابطَ يكذب على إنسانٍ ثالث.
     ٣) **ولا يُحذف طلبٌ قيدَ نظر.** المنتهيةُ والمعتمَدةُ وحدها
        (`purgeable.ts`) — وما بينهما قرارٌ لم يُبَتّ، وحذفُه يمحو طلبَ
        إنسانٍ ينتظر جوابا.
     ٤) **ولا يُمحى من له عقدٌ نافذ.** وتفصيلُه عند موضعه أدناه.

     > ⚠️ وكان هنا حارسٌ خامسٌ — «من صار مدرّبا لا يُحذف طلبُه» — **حُذف
     > بقرار صاحب المنصّة (١٤ سبتمبر ٢٠٢٦)**، وبقي مكتوبا في هذا الرأس بعده
     > فصار يصف رمزا لا وجودَ له. ومن قرأه وأراد حارسا رابعا كان يبني على
     > قاعدةٍ نُسخت. فصُحّح (١٩ سبتمبر ٢٠٢٦) مع دخول الحارس الرابع. */
  ...
  async purge(
    ...
    /* ═══ ولا يُمحى من وقّع عقدا نافذا ═══

       وهذا **ليس نقضا** لقرار صاحب المنصّة (١٤ سبتمبر ٢٠٢٦) الذي فتح المحوَ
       للمدرّب المعتمَد. ذاك القرارُ قام على مقدّمةٍ صرّح بها: «لم نتعاقد مع
       أحدٍ إطلاقا». والمقدّمةُ تنتهي في اللحظة التي يُعتمَد فيها أوّلُ عقدٍ
       موقَّع — وهذا الحارسُ هو تلك اللحظةُ بعينها، لا رأيا ثانيا في قراره.
       وقد قال تعليقُ الحارس الآخر ما يوافقه: «فإن وقع فمعناه أنّ في البيانات
       ما لا يتوقّعه، والوقوفُ حينها أصوبُ من المضيّ».

       وهو من جنس حارسِ الشهادات لا من جنس سجلّنا عن المدرّب: العقدُ الموقَّعُ
       المعتمَدُ **وثيقةُ الطرف الآخر** كما هو وثيقتُنا — فيه توقيعُه وهاشُ
       ما وقّع عليه وعنوانُ شبكته. ومحوُه يمحو دفاعَه كما يمحو دفاعَنا، وهو
       ما لا يملكه طرفٌ عن طرف.

       والمنفذُ الذي يسدّه ليس نظريّا: من وقّع ثمّ انسحب يصير طلبُه
       `withdrawn` — وهي في `PURGEABLE_STATUSES` — فيُمحى عقدُه النافذُ معه
       بضغطة. والموقوفُ كذلك.

       ولا يُشترط أن يكون الملفُّ قائما: الصفوفُ تُقرأ من العقود مباشرةً. */
    if (app.profile) {
      const live = await this.prisma.trainerContract.count({
        where: { profileId: app.profile.id, status: 'countersigned' },
      })
      if (live > 0) {
        throw new AuthError(
          'effective_contract',
          `لهذا المدرّب ${live} ${live === 1 ? 'عقدٌ نافذٌ موقَّعٌ ومعتمَد' : 'عقودٍ نافذةٍ موقَّعةٍ ومعتمَدة'} — `
          + 'وفيها توقيعُه ودليلُه لا سجلُّنا وحدَه، فلا تُمحى بقرارٍ من طرفٍ واحد. '
          + 'أوقِف ملفَّه، أو افسخِ العقدَ بمسار الرحيل ثمّ احذفه.',
          409,
        )
      }
    }
