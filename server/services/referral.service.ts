...
export class ReferralService {
  ...
  async wideLinkFor(userId: string): Promise<{ code: string; slug: string; url: string; publicReady: boolean }> {
    ...
    return {
      code: row.code,
      slug,
      /* ═══ ولماذا لا يُرمَّز المسارُ هنا ═══

         `public-slug.ts` يقول قصدَه صراحةً: «الحروفُ العربيّةُ تبقى عربيّةً
         في العنوان — لا تُنقحر… والقارئُ العربيُّ يقرأ اسمَ مدرّبه». وكان
         `encodeURIComponent` ينقض ذلك عند آخر خطوة: يصل المدرّبَ في خانة
         النسخ `‎/t/%D8%B1%D8%A7%D9%85%D9%8A-%D8%A7%D9%84…`‎ — سطرٌ لا يُقرأ
         ولا يُرسَل في رسالةٍ ولا يُعرف صاحبُه منه. فالقصدُ كان محفوظا في
         الاشتقاق ومهدورا في العرض.

         ولا خطرَ في تركه خامّا: `KEEP` في `public-slug` لا يقبل إلّا حرفا
         أو رقما أو شرطة — فلا `/` ولا `?` ولا `#` ولا مسافةٌ ولا `%` تدخل
         مسارا. والمتصفّحُ يرمّزه عند الإرسال، والخادمُ يفكّه عند الاستقبال،
         فالشكلان يصلان الطريقَ نفسَه — جُرّبا معا. */
      url: `${publicSiteUrl()}/t/${slug}`,
