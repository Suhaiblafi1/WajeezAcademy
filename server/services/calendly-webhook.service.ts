...
export class CalendlyWebhookService {
  ...
  async handle(input: CalendlyWebhookEvent): Promise<{
    ...
    /* ═══ ولماذا الشرطان معا، ورقمُ الطلب وحدَه أيسر ═══

       سُئل عن هذا مرّتين في يومٍ واحد (١٣ سبتمبر ٢٠٢٦): حجزٌ حقيقيٌّ تُجووز
       لأنّ البريدَ عند الحجز خالف بريدَ الطلب، فعُرض تخفيفُ الشرط إلى الرقم
       وحدَه. وقرارُ صاحب المنصّة: **يبقى الشرطان**.

       وعلّتُه أنّ `WJ-TR-2026-000NN` متسلسلٌ يُخمَّن في محاولةٍ أو اثنتَين:
       فمن حزر رقما ألصق حجزَه بطلب إنسانٍ آخر — يُنقل الطلبُ إلى
       `interview_scheduled`، ويُرسَل ملفُّ المتقدّم كاملا (تخصّصاتُه ووثائقُه)
       إلى لجنة المراجعة على أنّه صاحبُ الموعد. فالبريدُ هو الحمايةُ الوحيدةُ
       القائمة، وحذفُه يفتح هذا البابَ لأجل راحةٍ في حالةٍ نادرة.

       والثمنُ مقبولٌ لأنّه صار **مقولا**: المتجاوَزُ يُعَدّ في `failed` ويُسمّى
       سببُه في الأثر وفي صحّة النظام — فلا يضيع صامتا كما ضاع أوّلَ مرّة. */
