...
export function registerAdminUserRoutes(app: FastifyInstance, prisma: PrismaClient, auth: AuthService) {
  ...
  app.delete('/api/admin/users/:id', {
    ...
    /* ═══ والبريدُ قبل المحو كالأثر (ي-٤) ═══
       ...
       ═══ وهذا البابُ يُرسِل مباشرةً، والدفعةُ تكتب في الطابور (ي-٦) ═══

       والحدُّ بينهما قاعدةٌ لا سهو: **ما يراه إنسانٌ ينتظر جوابَه يخرج في
       حينه، وما لا يُرى يُوضَع في طابورٍ يُعيد المحاولة.**

       فحذفٌ مفردٌ يقف عليه موظّفٌ أمام شاشة: يُخبَر بنتيجة الإرسال في حينها
       فيتصرّف — ورسالةٌ واحدةٌ ليست دفعةً تتجاوز حدَّ المزوّد. أمّا دفعةُ
       مئتَي حساب، فلا يقرأ أحدٌ مئتَي نتيجة؛ ولذلك تُكتب في `OutboxMail`
       ويُعيد العاملُ ما سقط بلا أن يطلب أحدٌ ذلك. */
