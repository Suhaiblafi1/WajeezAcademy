...
import { registerCohortFileRoutes } from './routes/cohort-file.routes'
...
export async function buildApp(prisma: PrismaClient) {
  ...
  /* ═══ والبايتاتُ الخام: بلا هذا المحلّل لا يُرفع ملفٌّ واحد ═══

     ─────────── العطبُ الذي كُتب له ───────────

     Fastify لا يعرف من الأنواع إلّا ما سُجّل له محلّل، وما سواه يردّه
     **قبل أن يبلغ المعالجَ أصلا** بـ٤١٥ `FST_ERR_CTP_INVALID_MEDIA_TYPE`.
     ولم يكن مسجَّلا هنا إلّا `application/json`.

     و`PUT /api/v1/uploads/:key` يقرأ `req.body as Buffer`. وحدُّ العطب قِيس
     بإزالة هذا المحلّل ومراقبةِ ما يبلغ المعالج (`upload-parser.test.ts`):

       · `application/octet-stream` → يبلغ — لـFastify ٥ محلّلٌ له أصلا
       · `image/*` · `application/pdf` · `video/mp4` → ٤١٥ قبل المعالج

     فالشاشاتُ التي ترسل `octet-stream` كانت تعمل (وثيقةُ المتقدّم، والسيرةُ
     الذاتيّة، ومادّةُ الشعبة، والتسجيل). **والمكسورُ ما أعلن نوعَه الحقيقيّ**:
     `TrainerRunOps.tsx` يرسل `content-type: file.type`، فكان رفعُ صورة
     المدرّب يُردّ عند الإطار ولا يُشغَّل سطرٌ ممّا كُتب له.

     ولم يكشفه اختبارٌ لأنّ الاختباراتِ كانت تنادي الخدماتِ مباشرةً، والمسارُ
     يُقاس بـ٤٠٤ و٤٠٣ لا بنجاحِ رفعٍ حقيقيّ.

     ─────────── ولماذا `*` لا قائمةُ أنواع ───────────

     المرفوعُ ستّةُ نماذجَ بأنواعٍ لا تُحصى مقدّما: صورٌ وPDF ومستنداتٌ
     وتسجيلاتٌ صوتيّةٌ ومرئيّة. وقائمةٌ تُعدّ باليد تنسى واحدا فيسقط رفعُه
     بـ٤١٥ غامضةٍ لا يفهمها صاحبُه.

     والسقفُ لا يضيع بهذا: `bodyLimit` على المسار نفسِه هو الذي يحدّ، وفحصُ
     `resolveStorageOwner` يردّ كلَّ مفتاحٍ بلا مالك، وتوقيعُ البايتات يردّ
     ما ادّعى أنّه صورةٌ وليس بها. فالبابُ مفتوحٌ للبايتات ومحروسٌ بثلاثة. */
  ...
  registerCohortFileRoutes(app, prisma)
