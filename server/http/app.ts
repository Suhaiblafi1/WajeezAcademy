...
export async function buildApp(prisma: PrismaClient) {
  ...
  const app = Fastify({
    /* ═══ عنوانُ العميل الحقيقيّ (المهمّة ١٧) ═══
       ...
       من الإنترنت مباشرةً في أيّ بيئة: هو اليومَ عمليّةُ Node على Cloudways
       خلف Apache، وكان قبلها دالّةً خلف وسيط Vercel. فكان عنوانُ الوسيط هو
  ...
  /* ترويسات أمان على كل استجابة (API + /docs) — تعمل في الدالة السحابية أيضا:
     ...
     `public/.htaccess` (كانت في `vercel.json` قبل الانتقال إلى Cloudways) */
