...
export async function buildApp(prisma: PrismaClient) {
  ...
  /* ═══ والبايتاتُ الخام: بلا هذا المحلّل لا يُرفع ملفٌّ واحد ═══
     ...
     **قبل أن يبلغ المعالجَ أصلا** بـ٤١٥ `FST_ERR_CTP_INVALID_MEDIA_TYPE`.
     ...
     و`PUT /api/v1/uploads/:key` يقرأ `req.body as Buffer` — فكلُّ رفعٍ في
     المنصّة كان يُردّ: صورةُ المدرّب، وصورةُ الحساب، والسيرةُ الذاتيّة،
     وموادُّ الشعبة، والتسجيلاتُ، وملفُّ التسليم. يرسل المتصفّحُ
     `content-type: image/webp` فيسقط الطلبُ عند الإطار، ولا يُشغَّل سطرٌ
     واحدٌ ممّا كُتب للرفع.

     ولم يظهر ذلك في اختبارٍ قطّ لأنّ الاختباراتِ كانت تنادي الخدماتِ
     مباشرةً، والمسارُ يُقاس بـ٤٠٤ و٤٠٣ لا بنجاحِ رفعٍ حقيقيّ. وهو بعينه ما
     يحذّر منه رأسُ `storage.service.ts`: «رفعُ السيرة الذاتيّة لم يُرسل
     الملفَّ وقال للطالب: رُفعت».
