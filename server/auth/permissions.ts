...
export const PERMISSIONS = [
  ...
  /* ═══ اعتمادُ ما قدّمتَه بنفسك — استثناءٌ مقصودٌ لا ثغرة ═══

     maker-checker قاعدةٌ حسنةٌ حين يكون في الغرفة اثنان. وليس في هذه المنصّة
     اليومَ إلّا واحد: صاحبُها يؤلّف الدورةَ ويقدّم طلبَها ثمّ يقف أمام ٤٠٣
     ينتظر شخصا ثانيا لا وجودَ له — فتبقى الدورةُ مسوّدةً إلى الأبد،
     **والحوكمةُ التي وُضعت تمنع الخطأَ صارت تمنع العمل**.

     وقرارُ صاحب المنصّة (٧ سبتمبر ٢٠٢٦) يحكم هذا بعينه: «لا أريد شيئا يعيق
     تطويرَ الموقع والمنصّات»، والبوّابةُ الوحيدةُ آليّةٌ لا بشريّة.

     ولم تُنزع القاعدةُ بل قُيّد استثناؤها في ثلاثة:
     · **حبّةٌ لا دور** — فلا قرارَ صلاحيّةٍ يُبنى على اسم الدور وحدَه، كما
       يقول رأسُ هذا الملفّ. ومن دون المدير الأعلى لا يملكها:
       `academic_manager` يراجعه غيرُه كما كان.
     · **ولا تُفوَّض** (`NON_DELEGATABLE`) — وإلّا صارت بابا يمنح به الموظّفُ
       نفسَه ما مُنع منه، وهو عينُ ما تمنعه قاعدةُ التفويض أسفلَه.
     · **ويُكتب وقوعُها** — كلُّ اعتمادِ ذاتٍ يُسجَّل في سجلّ الأثر باسمه
       صريحا، فيُقرأ في `/admin/audit` ولا يُستخرَج بعد شهرٍ بمقابلة عمودَين. */
  { key: 'catalog.self_approve', description: 'اعتمادُ طلبِ تغييرٍ قدّمتَه بنفسك — استثناءُ maker-checker' },
  // التشخيص
...
/* ─────────── قيامُ الحسابِ على المنصّة: أهو مدرّبٌ فحسب؟ ───────────

   المدرّبُ الخالصُ **بوّابتُه هي دخولُه**: لا شاشةَ له سوى `/trainer`، فإيقافُ
   تدريبِه وإيقافُ حسابِه شيءٌ واحدٌ عنده. وليس كذلك من جمع إلى التدريب موقعا
   آخر — مديرا أو مستشارا أو ماليّةً أو دعما: لحسابه قيامٌ لا يمرّ بالتدريب
   أصلا، فإيقافُ تدريبِه لا يُسقط ذلك القيام.

   والفصلُ على الرتبة لا على تعداد الأدوار بالاسم: `trainer` وما دونَه
   (`trainer_applicant` · `learner`) حالاتٌ في مسار التدريب والتعلّم، وما
   فوقَه موقعٌ في الأكاديمية. فدورٌ جديدٌ يُضاف غدا يقع في جهته من تلقائه. */
export function holdsRoleBeyondTrainer(roles: readonly string[]): boolean {
  return roles.some((r) => (ROLE_RANK[r] ?? 0) > ROLE_RANK.trainer)
}
...
/* ── حبّاتٌ لا تُفوَّض بحالٍ (البند ٦٦) ──
   ...
   تُستثنى صراحةً — لا لأنّ التفويضَ سيّئ، بل لسببَين مختلفَين:

   · **ما يمحو المنصّةَ ولا رجعةَ فيه** — `admin.accounts.reset` و
     `admin.users.purge_bulk`. فمن ملكها يجب أن يملكها بدوره لا بمنحةٍ من
     غيره، وتبقى في `super_admin` وحدَه حيث يراها Git ويراجعها.

   · **وما يرفع حاجزَ فصلِ الأعمال نفسَه** — `catalog.self_approve`. وهذه
     أوضحُ من أختَيها: حبّةٌ تُبطل قاعدةَ «لا يعتمد المرءُ ما قدّمه» لو
     فُوِّضت لَصار المنعُ كلُّه اسما — يمنحها من عليه الحاجزُ لمن يشاء، أو
     لنفسه بيدِ من فوقه. فالتفويضُ ههنا يهدم ما تحرسه القاعدةُ أسفلَه. */
const NON_DELEGATABLE: readonly string[] = ['admin.accounts.reset', 'admin.users.purge_bulk', 'catalog.self_approve']
