#!/usr/bin/env node
/* مُشغِّلُ العامل الخلفيّ — عمليّةٌ واحدةٌ إلى جانب الخادم.

   كُتب هذا الملفُّ ولا موضعَ لتشغيله: كانت المنصّةُ على Vercel، ودالّتُها
   تستيقظ للطلب وتنام بعده — فلا مكانَ لحلقةٍ تعمل كلَّ دقيقة، وبقيت الوعودُ
   في الطابور. **وذلك المانعُ زال**: المنصّةُ على خادمٍ نملكه، والخادمُ حاويةُ
   Node دائمةٌ تحت Docker، وهذا الملفُّ عمليّةٌ ثانيةٌ إلى جانبها — لا خدمةٌ
   خارجيّةٌ ولا مكتبةُ طوابير.

   وله خدمةُ `worker` في `deploy/compose.prod.yml` منذ ٦ سبتمبر: الصورةُ
   نفسُها بأمرٍ آخر، تُقلع مع أوّل نشرة.

   ⚠️ **وكان مانعٌ ثانٍ حال دون تشغيله: قناةُ البريد.** تشغيلُ العامل وقناةُ
   البريد مغلقةٌ كان يُحوّل الطابورَ كلَّه إلى `failed` في دورةٍ واحدة، ووظيفةُ
   الإرسال لا تنظر إلى `failed` ثانيةً أبدا. **ورُفع بالشيفرة لا بالانتظار**:
   `dispatchQueuedNotifications` تسأل عن القناة قبل الصفّ (`liveChannels`)،
   فما لا مزوّدَ له يبقى `queued` ينتظر وصلَ قناته ويُقال عددُه في الخبر
   (`docs/DEPLOYMENT.md` §١٠).
