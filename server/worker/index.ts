...
/* مُشغِّلُ العامل الخلفيّ — عمليّةٌ واحدةٌ إلى جانب الخادم.
   ...
   ⚠️ **وزوالُ المانع ليس تشغيلا.** لا خدمةَ لهذا الملفّ في
   `deploy/compose.prod.yml` ولا `WORKER_ENABLED` في بيئة الإنتاج — فهو إلى
   اليوم لا يعمل هناك، والطابورُ يُملأ ولا يُفرَّغ.

   ⚠️ **والمانعُ الباقي غيرُ المانعِ الأوّل: قناةُ البريد.** تشغيلُ العامل
   وقناةُ البريد مغلقةٌ يُحوّل الطابورَ كلَّه إلى `failed` في دورةٍ واحدة،
   ووظيفةُ الإرسال لا تنظر إلى `failed` ثانيةً أبدا — فالتعافي يدويٌّ صفّا
   صفّا. الترتيبُ الآمنُ في `docs/DEPLOYMENT.md` §١٠: بريدٌ يصل الوارد، ثمّ
   دورةٌ واحدةٌ للفحص، ثمّ الحلقة.
