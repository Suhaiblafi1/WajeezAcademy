...
# ── والمساحةُ تُفحَص قبل أن يُلمَس شيء ──
#
# قرصٌ ممتلئٌ يُوقف القاعدةَ والبناءَ معا: فيردّ الخادمُ `EAI_AGAIN` على كلّ
# مسارٍ يمسّ القاعدة، وتقف كلُّ نشرةٍ عند الخطوة ٣ بتصميمها — والصفحاتُ
# الساكنةُ تبقى تُخدَم من الحاويات القديمة. فالموقعُ يبدو حيّا وواجهتُه
# ميّتة، ولا نشرةَ تصل. وقع ليلةَ ٢٣ سبتمبر ٢٠٢٦ عشرَ ساعات.
#
# والفحصُ هنا لا بعد الخطوة ٢: النسخةُ الاحتياطيّةُ نفسُها تكتب على القرص،
# فتسقط بسبب الامتلاء وتقول «تعذّرت النسخة» — وهي رسالةٌ تُرسل الباحثَ إلى
# غير موضع العلّة. ورأسُ preflight-disk.sh يشرح ما وقع وما لا يُمَسّ.
bash deploy/preflight-disk.sh . || exit 1
...
# ── والقديمُ يُسقَط بعد أن تعمل الجديدة ──
#
# الصورةُ السابقةُ صارت معلَّقةً (بلا وسم) في اللحظة التي أخذت الجديدةُ
# وسمَها، وذاكرةُ البناء نمت معها. وبلا هذا السطر تتراكم نشرةً بعد نشرة حتّى
# يمتلئ القرص — وهو بعينه ما أوقف الإنتاج ليلةَ ٢٣ سبتمبر ٢٠٢٦.
#
# **وبعد التبديل لا قبلَه**: قبله تكون الصورةُ القديمةُ هي التي تخدم، فمسحُها
# يقطع الخدمةَ إن تعثّر التبديل. وبعده لا يشير إليها شيء.
#
# ولا `--volumes` ولا `-a` هنا: الأوّلُ يحذف `pgdata` متى وقفت حاويةُ
# القاعدة، والثاني يحذف كلَّ صورةٍ لا تستعملها حاويةٌ قائمة — ومنها صورُ
# القاعدةِ وCaddy المسحوبةُ من المستودَع، فتُجلَب من جديد في كلّ نشرة.
# فالمسحُ على المعلَّق وحدَه: ما لا وسمَ له ولا يشير إليه شيء.
