...
# ── والقرصُ يُسأل قبل أن يُلمَس شيءٌ كذلك ──
#
# امتلأ القرصُ في منتصف نشرةٍ (٢٤ سبتمبر ٢٠٢٦) فسقطت القاعدةُ عشرَ ساعات:
# النشرةُ أسقطت الموقعَ الذي جاءت لتحدّثه. فتُرفض قبل أن تبدأ — وقبل السحب
# عمدا: فلا يتقدّم الفرعُ المحلّيّ، ويعيد المراقبُ المحاولةَ كلَّ دورةٍ مُنذِرا،
# حتّى تُفرَّغ المساحةُ فتمضي النشرةُ وحدَها.
...
# ── وما تتركه كلُّ نشرةٍ على القرص يُكنَس بعد نجاحها ──
#
# كلُّ بناءٍ يترك صورتَه السابقةَ بلا اسم، وطبقاتٍ في ذاكرة البناء لا يمحوها
# شيء. وبلغت ذاكرةُ البناء وحدَها ١١ غيغابايتا يومَ امتلأ القرص (٢٤ سبتمبر
# ٢٠٢٦) — ولم يكن في المستودَع أمرُ تنظيفٍ واحد.
#
# ويقع هنا لا قبل البناء: بعد أن أجاب الخادمُ الجديدُ داخليّا، فالصورةُ
# القائمةُ في يد حاوياتٍ تعمل ولا تُمسّ. وذاكرةُ اليوم الأخير تبقى، فالنشرةُ
# التالية لا تُعيد `npm ci` من أوّله.
#
# ⚠ ولا يُكتب هنا تنظيفٌ يمسّ الأحجام — فيها القاعدةُ والملفّاتُ المرفوعة.
# وتعذُّرُ الكنس لا يُسقط نشرةً نجحت: يُقال ويُمضى.
docker image prune -f >/dev/null 2>&1 \
  || printf '\033[33m⚠️  تعذّر حذفُ الصور القديمة — راجع: docker system df\033[0m\n' >&2
docker builder prune -f --filter until=24h >/dev/null 2>&1 \
  || printf '\033[33m⚠️  تعذّر كنسُ ذاكرة البناء — راجع: docker system df\033[0m\n' >&2
