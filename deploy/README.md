...
> **وخادمُ وجيز اليوم ليس على هذا الترتيب** (قرارُ صاحب المنصّة · ١٤ سبتمبر
> ٢٠٢٦): لا صندوقَ تخزين — مَقلَبٌ ليليٌّ على القرص، وحملُه خارجا على **نسخ
> Hetzner السحابيّة**، والسكربتُ يمرّ بـ`BACKUP_REMOTE_ALLOW_LOCAL=yes`.
> فالتحذيرُ المحلّيُّ الذي يطبعه متوقَّعٌ لا عطب، ولا يُصلَح بضبط
> `BACKUP_REMOTE` على وجهةٍ بعيدة.
>
> وما تحتَه وصفةُ **خادمٍ جديدٍ أو انتقالٍ إلى وجهةٍ خارجيّة** — لا وصفُ ما
> يجب أن يكون هنا اليوم. وحدُّ الترتيب القائم — نسخُ Hetzner تموت مع الخادم —
> في `docs/رسالة-النسخ-الاحتياطي.md` §٠.

`deploy/backup.sh` **يرفض العمل** بلا وجهةٍ خارجَ الخادم أو إذنٍ صريحٍ يقوم
مقامَها. نسخةٌ على القرص نفسِه ليست نسخةً احتياطية: عطبُ القرص يأخذ الأصلَ
والنسخةَ معا.
...
> ### ⚠️ صُحّح في ٧ سبتمبر ٢٠٢٦ — كان العنوانُ هنا خاطئا
>
> كان المسارُ مكتوبا مقلوبَ الترتيب — `payments` قبل `webhook`، بلا جمعٍ ولا
> اسمِ مزوّد — **ولا وجودَ له في الشيفرة**. والمسجَّلُ فعلا
> `‎/api/webhooks/payments/:provider`
> ([`commerce.routes.ts:220`](../server/http/routes/commerce.routes.ts)).
>
> ومن سجّله كما كان، أرسل سترايبُ إليه فردّ **٤٠٤** — فلا تسويةَ ولا تسجيل،
> **والمالُ مقبوض**. وهو بعينه ما يحذّر منه عنوانُ هذا القسم. ويحرسه الآن
> [`src/tests/deploy/payment-webhook-url.test.ts`](../src/tests/deploy/payment-webhook-url.test.ts):
> يقرأ المسارَ من الشيفرة ويقابله بما تكتبه الوثائق.

١. أضف نقطة webhook جديدة: `https://<نطاقك>/api/webhooks/payments/stripe`
...
الرابطُ وحدَه يعرض التقويم، لكنه لا يُدخل الموعدَ الحقيقيَّ في وجيز. لذلك
يلزم اشتراك webhook واحدٌ على النطاق الحيّ:
...
لا تُنشئ المقابلةَ رسالةُ المتصفّح؛ هي تغيّر رسالةَ النجاح في الصفحة فقط.
الكتابةُ من webhook الموقّع على الجسم الخام، وتُرفض البصمةُ بعد ثلاث دقائق.
...
الخادم منه**. فقُلب الاتّجاه: الخادمُ يسأل `main` كلَّ دقيقة، فإن تحرّكت
