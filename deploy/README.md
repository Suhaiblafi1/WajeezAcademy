...
## ٣ · النسخُ الاحتياطيّ — القرارُ القائم

**لا صندوقَ تخزين.** قرارُ صاحب المنصّة (١٤ سبتمبر ٢٠٢٦): المعوَّلُ عليه
**نسخُ Hetzner السحابيّة** — صورُ الخادم كاملا، مُفعَّلةٌ عليه — ومعها
مفرغاتٌ ليليّةٌ محلّيّةٌ يكتبها `deploy/backup.sh`.

ولذلك في ملفّ البيئة على الخادم:

```bash
BACKUP_REMOTE_ALLOW_LOCAL=yes
```

وهو **تعطيلٌ مقصودٌ للحارس لا سهوٌ يُصلَح**. الحارسُ يرفض وجهةً محلّيّةً لأنّ
نسخةً تموت مع القرص ليست نسخة — والجوابُ هنا أنّ النسخةَ الحقيقيّةَ خارجَ
القرص أصلا: نسخةُ Hetzner السحابيّة. والمفرغُ المحلّيّ راحةُ استرجاعٍ سريعٍ فوقها، لا
هو النسخة.

### وما لا يقوله «--verify» بعد هذا القرار

`bash deploy/backup.sh --verify` يسترجع **المفرغَ المحلّيّ** ويثبته. وهو
إثباتٌ صادقٌ لما يفعله، **وليس إثباتا لمسار التعافي الفعليّ**. فمن قرأ خضرتَه
على أنّها «جُرّب استرجاعُ Hetzner» أخطأ — وهذا بعينه حارسٌ أخضرُ لسببٍ خاطئ.

**ومسارُ التعافي الحقيقيُّ يُختبر في لوحة Hetzner**: استرجاعُ نسخةٍ إلى خادم،
ثمّ فتحُ الموقع. **ولم يُختبر بعدُ** — ونسخةٌ لم يُرَ استرجاعُها ليست نسخةً
حتّى يُرى.

و`deploy/status.sh` يطبع «تعطيلُ الحارس ⚠» منذ اليوم دائما. وهو صوابٌ لا عطب:
يقول ما هو قائمٌ فعلا.

### والنسخةُ ليست اللقطة — وفرقُهما في الحذف

**نسخُ Hetzner السحابيّة** (Cloud Backups) **مربوطةٌ بالخادم**: حذفُ الخادم
يحذفها معه. واللقطةُ (`snapshot`) وحدَها تبقى بعده، وهي تُنشأ باليد. فالكلمتان
تُستعملان مترادفتَين في الحديث، والفرقُ بينهما هو الفرقُ بين نسخةٍ تنجو من حذف
الخادم ونسخةٍ لا تنجو.

| الحادث | مغطّى؟ |
|---|---|
| عطبُ القرص · فسادُ البيانات · هجرةٌ خاطئة · حذفُ صفوفٍ بالخطأ | **نعم** |
| حذفُ الخادم نفسِه · ضياعُ الوصول إلى حساب Hetzner | **لا** |

وهو حدُّ الترتيب لا نقضُه — يُكتب ليُعرَف حين يُسأل عنه، لا ليُستأنَف به
القرار. ومن أراد النجاةَ من حذف الخادم فأرخصُ سبيلٍ لقطةٌ يدويّةٌ قبل العمل
الخطر، لا صندوقُ تخزين. والتفصيلُ في `docs/رسالة-النسخ-الاحتياطي.md` §٠.

### لو عُدل عن القرار

`backup.sh` يقبل أيَّ وجهةِ rclone بصيغة «اسم:مسار» (S3 · Backblaze B2 ·
صندوق تخزين). تُنشأ على الخادم، ثمّ يُرفع `BACKUP_REMOTE_ALLOW_LOCAL`:

```bash
rclone config create wajeez-backup sftp \
  host <المضيف> user <المستخدم> port 23 \
  pass "$(rclone obscure '<كلمة-السرّ>')"
rclone lsd wajeez-backup:          # أثبِتها قبل الوثوق بها
# ثمّ في ملفّ البيئة: BACKUP_REMOTE=wajeez-backup:wajeez
```

وعلى الخادم اليومَ وجهةُ rclone معرَّفةٌ غيرُ مستعملة — تُركت بلا ضرر.
...
ثم فعّل النسخة الليلية **وإثباتَ استرجاعها** — مؤقّتان لا واحد:

```bash
sudo cp deploy/wajeez-backup.* deploy/wajeez-verify.* /etc/systemd/system/
sudo systemctl daemon-reload
sudo systemctl enable --now wajeez-backup.timer wajeez-verify.timer
systemctl list-timers 'wajeez-*'
```

| المؤقّت | متى | ماذا يفعل |
| --- | --- | --- |
| `wajeez-backup` | كلَّ ليلة ٠٣:٠٠ بعمّان | يأخذ النسخة |
| `wajeez-verify` | كلَّ أحد ٠٤:٠٠ بعمّان | **يسترجعها في قاعدة خدشٍ ويعدّ صفوفَها** |

وهما شيئان لا شيءٌ واحد: الأوّلُ يفشل فتضيع نسخةُ ليلة، والثاني يفشل فيتبيّن
أنّ **النسخَ كلَّها** لا تُسترجَع.

> **ولماذا أسبوعيٌّ والإثباتُ يصلح شهرا:** الإثباتُ يشيخ بعد ٣١ يوما
> (`MAX_AGE_DAYS`)، وعليه تتوقّف **إعادةُ ضبط الحسابات** — محوٌ لا رجعةَ فيه.
> فمؤقّتٌ شهريٌّ يجعل كلَّ تشغيلةٍ فرصةً وحيدة: تسقط واحدةٌ فيشيخ الإثباتُ
> ويُغلق البابُ بلا عطبٍ حقيقيّ. والأسبوعيُّ يعطي أربعَ فرص.

وحالُه يُقرأ في **صحّة النظام** («آخرُ استرجاعٍ مُثبَت») — يصفرّ قبل الشيخوخة
بعشرة أيّام، فلا تُفاجأ ببابٍ مغلق.
...
**قبل أن ترى هذا السطر مرّة واحدة، ليس عندك نسخٌ احتياطي.**

وبعدها لا تُعده بيدك: `wajeez-verify.timer` يعيده كلَّ أحد، وصحّةُ النظام
تقول متى جرى آخرَ مرّة.
