...
#
# ── ✅ وقرارُ صاحب المنصّة (١٤ سبتمبر ٢٠٢٦): لا صندوقَ تخزين ──
#
# ما فوقُ **تاريخٌ لا وصفٌ للحال**. فالمضبوطُ على الإنتاج اليومَ هو المسارُ
# المحلّيُّ نفسُه — `/var/backups/wajeez` — ومعه `BACKUP_REMOTE_ALLOW_LOCAL=yes`
# و`BACKUP_KEEP_DAYS=30`. **عن قصدٍ لا سهوا.**
#
# ولمَ لم يعد عطبا: العلّةُ كانت أنّ النسخةَ لا تغادر الآلةَ أصلا. واليومَ
# تُفعَّل **نسخُ Hetzner السحابيّة** على `ubuntu-4gb-nbg1-1`، وهي تلتقط قرصَ
# الخادم كلَّه وفيه هذا المجلّد — فتغادر النسخةُ بطريقٍ آخرَ لا بـ`rclone`.
#
# والإثباتُ جرى لا افتُرض: `--verify` أخضرُ، واسترجاعُه أعاد ٧ مستخدمين و٣
# طلبات و١٢ كائنَ تخزين. فبابُ «إعادة ضبط الحسابات» مفتوحٌ بإثباتٍ حقيقيّ.
#
# ⚠️ فلا «يُصلَح» هذا: من قرأ ما فوقَه وحدَه ظنّ الإنتاجَ مضبوطا خطأً فأعاده
#    إلى وجهةٍ بعيدة. والمخاطرةُ الباقيةُ معلومةٌ ومقبولة — النسخُ في مشروع
#    Hetzner نفسِه. والوجهةُ الوهميّةُ `[wajeez-backup]` باقيةٌ غيرَ مستعمَلة.
#
