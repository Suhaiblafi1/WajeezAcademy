...
if (process.env.GALLERY_OUT) {
  ...
  const items = [...[...pick.entries()].map(([file, v]) => ({ file, cap: v.cap })), ...jshots].filter((x) => existsSync(x.file))
  let total = 0
  const cards = items.map((x) => { const b = readFileSync(x.file); total += b.length; return `<figure><img loading="lazy" src="data:image/jpeg;base64,${b.toString('base64')}" alt=""><figcaption>${x.cap.replace(/</g, '&lt;')}</figcaption></figure>` }).join('\n')
...
html{direction:rtl}body{margin:0;background:var(--bg);color:var(--ink);font-family:"IBM Plex Sans Arabic",Tahoma,sans-serif;font-size:16px;line-height:1.8}main{max-width:1200px;margin:0 auto;padding:32px 20px 64px}h1{font-size:28px;margin:0 0 6px}p.lede{color:var(--muted);margin:0 0 24px}
...
<main><h1>معرض جولة الأدوار — أكاديمية وجيز</h1><p class="lede">${items.length} لقطة من ${F.results.length}: كلُّ شاشةٍ عليها ملاحظة، وشاشةٌ لكلّ مسارٍ بأعلى دورٍ يراه، وخطواتُ الرحلات العشر. ${F.generatedAt.slice(0, 10)}.</p><div class="grid">${cards}</div></main>`
