#!/usr/bin/env bash
# ⚠️⚠️  هذا **ليس** ناشرَ الإنتاج. لا تُشغّله على الخادم الحيّ.  ⚠️⚠️
#
# كُتب على أنّ الاستضافة Cloudways — كما كانت تقول `DEPLOYMENT.md`. والقياسُ
# كذّبها في ٦ سبتمبر ٢٠٢٦: `dig +short www.wajeezacademy.com` = 5.9.82.49
# (Hetzner)، والخادمُ حاويات Docker خلف Caddy. وهذا الملفُّ يفترض Supervisor
# و`npm start` على المضيف — فتشغيلُه هناك يفشل، أو أسوأُ: ينجح على خادمٍ آخر.
#
#   ناشرُ الإنتاج:  deploy/deploy.sh
#
# ولم يُحذف لأنّ أمرَ خادم Cloudways (إن وُجد: تجريبيٌّ؟ متروك؟) لم يُحسم بعد —
# والحذفُ بلا قياسٍ هو ما أوقعنا في هذا أصلا. يُحذف حين يُحسم.
#
# ── وما كان مكتوبا هنا أصلا ──
#
