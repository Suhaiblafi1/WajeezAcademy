...
#   ٣) يركّب سطرَ الجدولة، فيصير الخادمُ يسأل `main` كلَّ دقيقة وينشر وحدَه.
...
if printf '%s\n' "$CURRENT" | grep -qF "$WATCHER"; then
  ...
  ok "رُكِّب: كلَّ دقيقة يسأل الخادمُ main وينشر إن تحرّكت"
fi

step "٤/٤ · وبعد اليوم"
cat <<'AFTER'
لا شيءَ عليك. أيُّ تغييرٍ يصل main يظهر على الموقع خلال دقيقة.
...
ولا يُكتب في السجلّ شيءٌ حين لا جديد — بقصد: سطرٌ كلَّ دقيقة يُغرقه فلا
يُقرأ حين يهمّ.
AFTER

# ── وحالُ التنبيه يُقال هنا، لا يُترك ليُكتشف بعد فشلٍ صامت ──
#
# هذه اللحظةُ هي موضعُ القول: صار النشرُ آليّا، فمن يُخبرك حين يفشل؟
# وكان هنا سطرٌ ثابتٌ يقول «لا تنبيهَ بعد» — صار الآن ممكنا، فيُقاس ويُقال.
PING_SET="$(grep -E '^[[:space:]]*WAJEEZ_PING_URL=' deploy/.env.production 2>/dev/null \
  | tail -1 | cut -d= -f2- | tr -d '[:space:]' || true)"
if [ -n "$PING_SET" ]; then
  ok "والتنبيهُ مضبوط: تصل نبضةٌ كلَّ دورةٍ سليمة، و«/fail» فورَ فشلِ نشرة."
else
  cat <<'NOPING'

⚠️  ولا تنبيهَ مضبوطٌ بعد — وهذا أخطرُ ما يبقى.

    نشرةٌ تفشل في الثالثة فجرا تكتب سطرا في سجلٍّ لا يقرؤه أحد. وقد وقع:
    في ٢٠ سبتمبر ٢٠٢٦ فشلت النشرةُ اثنتي عشرة ساعةً كلَّ دقيقة، وبقي
    الموقعُ يخدم بناءً قديما، ولم يُعلَم إلّا بالسؤال.

    والعلاجُ دقيقتان:
      ١) أنشئ فحصا في https://healthchecks.io (مجّانيّ) بمهلة ٣٠ دقيقة
      ٢) ضع عنوانَه في deploy/.env.production:
           WAJEEZ_PING_URL=https://hc-ping.com/<المعرّف>
      ٣) لا شيءَ غيرُه — المراقبُ يقرؤه من الملفّ في الدورة التالية.
NOPING
fi
